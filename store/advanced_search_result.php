...
if(isset($_GET['keywords']) && $_GET['keywords'] != ''){
		...
		if($_GET['keywords'] == 'contact us'){
			tep_redirect(tep_href_link('contact-us'));	
		}
		elseif($_GET['keywords'] == 'weather'){
			tep_redirect(tep_href_link('weather'));	
		}
		elseif($_GET['keywords'] == 'shipping'){
			tep_redirect(tep_href_link('shipping'));	
		}
		elseif($_GET['keywords'] == 'free shipping'){
			tep_redirect(tep_href_link('shipping'));	
		}
		elseif($_GET['keywords'] == 'returns'){
			tep_redirect(tep_href_link('pages.php?page=return-policy'));	
		}
	...
	if(!isset($_GET['s'])){
		$pwstr_check = strtolower(substr($_GET['keywords'], strlen($_GET['keywords'])-1, strlen($_GET['keywords'])));
  	    if(($pwstr_check == 's') && ($_GET['keywords']!='vegas')){
  	            ...
  	            header('location: ' . ( FILENAME_ADVANCED_SEARCH_RESULT . '?search_in_keywords=1&plural=1&s=1&keywords=' . urlencode($pwstr_replace) . '' ));
  	            exit;
  	    }
        }
...
$query_exploded = explode(' ',$keywords);	  
    ...
    if (tep_not_null($query_exploded)) {
      if (!tep_parse_search_string($keywords, $search_keywords2)) {
        $error = true;

        $messageStack->add_session('search', ERROR_INVALID_KEYWORDS);
      }
    }
	$search_keywords =  $query_exploded ;
  }
...
<?php require(DIR_WS_INCLUDES . 'template-top-index.php'); ?>
   <div class="col-xs-12 form-horizontal form-group">
            <?php if (isset($HTTP_GET_VARS['plural']) && ($HTTP_GET_VARS['plural'] == '1')) {
            	  	echo 'Resuls:' . ' "<b><i>' . stripslashes($_GET['keywords']) . 's</i></b>"';
             	  } else {
            		echo 'Results:' . ' "<b><i>' . stripslashes($_GET['keywords']) . '</i></b>"';
             	  }
            ?>
    </div>
<?php
...
?>
        ...
        <div class="alpha" style="padding-bottom:20px;">
        <?php echo '<a href="' . tep_href_link(FILENAME_ADVANCED_SEARCH, tep_get_all_get_params(array('sort', 'page')), 'NONSSL', true, false) . '">'. '<button class="button-blue-small">Back</button>'.'</a>'; ?>
        </div>
    </div>
