...
class GoogleSitemap{
	...
	* @var string
	...
	var $savepath;
	/**
 	* $base_url is the URL for the STORE
	* @var string
 	*/
	var $base_url;
	/**
 ...
 */	
	function GoogleSitemap($host, $user, $db, $pass){
		...
		$this->filename = "sitemap";
		        ...
		        if (($pos = strpos(DIR_FS_CATALOG, "googlesitemap")) !== FALSE)   //fix for when path is gotten by dirname($HTTP_SERVER_VARS['SCRIPT_FILENAME'        {                                                               //it includes the google sitemap directory so remove it          $pos = strlen("googlesitemap");  //not returning right above - quick fix for now            $this->savepath = substr(DIR_FS_CATALOG, 0, - $pos - 1);            $this->savepath .= (substr($this->savepath, 0, -1) !== '/') ? '/' : ''; //append a slash if needed        }        else          $this->savepath = DIR_FS_CATALOG . (substr(DIR_FS_CATALOG, 0, -1) !== '/') ? '/' : '';

      
		$this->base_url = HTTP_SERVER . DIR_WS_HTTP_STORE;
		$this->debug = array();    
 ...
 */	
    ...
    $filename = "/domains/jupiterkiteboarding.com/public_html/store/". $this->savepath . $this->filename . $type;
      ...
      $file_check = file_exists($filename) ? 'true' : 'false';
		...
		switch($compress){
         ...
         if (GOOGLE_XML_SITEMAP_SHOW_DIAGNOSTIC == 'true')
           echo 'Opening   '.$filename. '<br>FS_CAT    '.DIR_FS_STORE. '<br>Server    ' . HTTP_SERVER . '<br>Save Path '. $this->savepath . '<br>WS_CAT    '. DIR_WS_HTTP_STORE.' <br>';
 ...
 */	
    ...
    $slash = substr(DIR_FS_STORE, 0 -1) == '/' ? 1 : 0;
    $path = (($pos = strpos(DIR_FS_STORE, "googlesitemap")) !== FALSE) ? substr(DIR_FS_STORE, 0, -strlen('googlesitemap') - $slash) : DIR_FS_STORE;
