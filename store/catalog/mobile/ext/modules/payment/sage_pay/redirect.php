...
// if the customer is not logged on, redirect them to the login page
  ...
  if ( isset($HTTP_GET_VARS['payment_error']) && tep_not_null($HTTP_GET_VARS['payment_error']) ) {
    $redirect_url = tep_mobile_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $HTTP_GET_VARS['payment_error'] . (isset($HTTP_GET_VARS['error']) && tep_not_null($HTTP_GET_VARS['error']) ? '&error=' . $HTTP_GET_VARS['error'] : ''), 'SSL');
  } else {
    $hidden_params = '';

    if ($payment == 'sage_pay_direct') {
      $redirect_url = tep_mobile_link(FILENAME_CHECKOUT_PROCESS, 'check=3D', 'SSL');
      $hidden_params = tep_draw_hidden_field('MD', $HTTP_POST_VARS['MD']) . tep_draw_hidden_field('PaRes', $HTTP_POST_VARS['PaRes']);
    } else {
      $redirect_url = tep_mobile_link(FILENAME_CHECKOUT_SUCCESS, '', 'SSL');
    }
  }
...
<base href="<?php echo (($request_type == 'SSL') ? HTTPS_SERVER : HTTP_SERVER) . DIR_WS_HTTP_MOBILE; ?>">
