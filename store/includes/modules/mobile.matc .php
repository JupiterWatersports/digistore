...
//We shall now create the disabled looking button... If it doesn't exits allready and is needed
	if(MATC_BUTTONSTYLE == 'gray' && version_compare(PHP_VERSION, "5") == -1){
		die('Your php version is to low to use gray buttons in the MATC extension for digistoe. Please change "Disabled buttonstyle" in your digistore admin panel.');
	}
	if  (MATC_BUTTONSTYLE == 'gray'){
		$TempImage = imagecreatefromgif(tep_mobile_button(IMAGE_BUTTON_CONTINUE));
		...
		imagegif($TempImage, tep_mobile_button(IMAGE_BUTTON_CONTINUE));
...
function warningoff(){
	$("#CAparagraph").animate({backgroundColor: "#cdcdcd"}, 200);
}
...
$(document).ready(function(){
	...
	if(MATC_BUTTONSTYLE == 'gray'){
		...
		echo tep_mobile_button(IMAGE_BUTTON_CONTINUE, 'id="TheDisabledButton" style="display:none;cursor:not-allowed;filter:alpha(opacity=33);-moz-opacity:.33;opacity:.33;" onMouseOut="warningoff()" onMouseOver="warningon()"'); 
	} ?>');
...
<tr>
	<td>
		<textarea name="conditions" bgcolor="#3c3c3c" class="small" rows="5" cols="30" readonly ><?php echo $textarea_contents; ?></textarea>
	</td>
</tr>
<?php 
