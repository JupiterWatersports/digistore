...
// 		print_r( $this->quotes );
	...
	function _getDmstcQuote($pkg, $method = '') {
		...
		foreach ($request_group as $service_group) {
			...
			$body = explode('<RateV4Response>',$body,2);
			$body = explode('</RateV4Response>',$body[1],2);
			$body = '<?xml version="1.0" encoding="UTF-8"?><RateV4Response>'.$body[0].'</RateV4Response>';
	...
	function _decode_domestic_response($service, $pkgvalue, $nonuspsinsurancecost, $opval) {
		...
		$qid = str_replace(array('&reg;', '&trade;', '&#174;', '&#8482;', ' 1-Day', ' 2-Day', ' 3-Day', ' Military', ' DPO'), '', $qname);
