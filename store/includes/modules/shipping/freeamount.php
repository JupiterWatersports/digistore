...
// class constructor
    function freeamount() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_FREEAMOUNT_ZONE > 0) ) {
        ...
        $check_flag = false;
        $check_query = tep_db_query("select zone_id, zone_country_id from " . TABLE_ZONES. " where zone_id >= '1' and zone_id <= '131'  and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
        ...
        while ($check = tep_db_fetch_array($check_query)) {
          ...
          if ($check['zone_id'] < 1) {
            $check_flag = true;
            break;
       } elseif ($check['zone_id'] == $order->delivery['zone_id'] && $check['zone_country_id'] == 223) {
            if (($check['zone_id'] != 70) && ($check['zone_id'] != 74) && ($check['zone_id'] != 75) && ($check['zone_id'] != 76) && ($check['zone_id'] != 77) && ($check['zone_id'] != 78) && ($check['zone_id'] != 79) && ($check['zone_id'] != 87))
            $check_flag = true;
            break;
          }
        }
...
if ($order->delivery['country_id'] != STORE_COUNTRY) { $this->enabled = false; }
}
...
// class methods
    function quote($method = '') {
      ...
      $cart_total = $cart->show_total();
        ...
        $check_if_sup_part1_query = tep_db_query ("select cb.products_id from customers_basket cb, customers c where c.customers_firstname = '".addslashes($order->delivery['firstname'])."' and c.customers_lastname = '".addslashes($order->delivery['lastname'])."' and cb.customers_id = c.customers_id");
	  ...
	  if ($cart_total < MODULE_SHIPPING_FREEAMOUNT_AMOUNT)
	  {
		if (MODULE_SHIPPING_FREEAMOUNT_DISPLAY == 'True')
		   {
			$this->quotes['error'] = MODULE_SHIPPING_FREEAMOUNT_TEXT_ERROR . $currencies->format(MODULE_SHIPPING_FREEAMOUNT_AMOUNT);
		   }
		$get_total = false;
	   } else {
	    $get_total = true;
      ...
      if ($shipping_weight > MODULE_SHIPPING_FREEAMOUNT_WEIGHT_MAX && $check_oversized < 1){
			if (MODULE_SHIPPING_FREEAMOUNT_DISPLAY == 'True')
			   {
	    	  	$this->quotes['error'] = MODULE_SHIPPING_FREEAMOUNT_TEXT_TO_HEIGHT . $currencies->format(MODULE_SHIPPING_FREEAMOUNT_AMOUNT);
			   }
			$get_weight = false;
	  } else {
	   $get_weight = true;
