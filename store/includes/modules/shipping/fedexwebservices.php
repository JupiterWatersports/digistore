<?php
/*
  FedEx Webservice Version 9_5 for osCommerce 2.2rc2a and later(?) -by dr_lucas 14/April/2014
...
//Class Constructor
  function fedexwebservices() {
    ...
    }	if(isset($_SESSION['disable_slow_shipping']) && $_SESSION['disable_slow_shipping'])$this->enabled = false;
...
//    $totals = $cart->show_total();  // this seems to work for most
    ...
    $request['RequestedShipment']['ShipTimestamp'] = date('c', strtotime("+" . MODULE_SHIPPING_FEDEX_WEB_SERVICES_SHIPPING_DAYS_DELAY. " days")); // 11/04/2014 -  Added MODULE_SHIPPING_FEDEX_WEB_SERVICES_SHIPPING_DAYS_DELAY days to the delivery calculations, to give us time to process the orders.
    ...
    $request['RequestedShipment']['Shipper'] = array('Address' => array(
                                                     ...
                                                     'StateOrProvinceCode' => utf8_encode(MODULE_SHIPPING_FEDEX_WEB_SERVICES_STATE),
                                                     'PostalCode' => utf8_encode(MODULE_SHIPPING_FEDEX_WEB_SERVICES_POSTAL),
    ...
    $request['RequestedShipment']['Recipient'] = array('Address' => array (
                                                       ...
                                                       'PostalCode' => utf8_encode($postcode), //customer postcode
...
//    $request['RequestedShipment']['RateRequestTypes'] = 'LIST';
    $request['RequestedShipment']['RateRequestTypes'] = MODULE_SHIPPING_FEDEX_WEB_SERVICES_RATES;
    $request['RequestedShipment']['PackageDetail'] = 'INDIVIDUAL_PACKAGES';
    $request['RequestedShipment']['RequestedPackageLineItems'] = array();
    
    $dimensions_failed = false;
    $new_shipping_weight = ''; //9.4.6 
...
//              $shipping_weight = $shipping_weight + ($shipping_weight*(isset($zc_tare_percent)/100)) + isset($zc_tare_weight); // is isset needed?
            }
...
// echo printRequestResponse($client);  // enable for troubleshooting only - output XML to screen
    if (MODULE_SHIPPING_FEDEX_DEBUGGING != 'off') {
    	if (MODULE_SHIPPING_FEDEX_DEBUGGING == 'header') {
        	printRequestResponse($client);  //24/03/2014 -  - For Debugging of XML output to the site header
      }elseif (MODULE_SHIPPING_FEDEX_DEBUGGING == 'file') {
          writeToLog($client);  //24/03/2014 -  - For Debugging of XML output to the file /includes/modules/shipping/fedextransactions.log
      }
    }
...
if (isset($response->RateReplyDetails[0])) {  //If more than 1 shipping option/rate given
...
////////////
    ...
    $transitTime = ''; // 9.4.6 
         ...
         // $transitTime = ' (Estimated Delivery on: ' . date('l, F jS Y \a\t g:ia', strtotime($rateReply->DeliveryTimestamp)) . ') ';
          $transitTime = ' (' . date('l, F jS Y', strtotime($rateReply->DeliveryTimestamp)) . ') ';
          }
  			$get_round1_shipping_method = strtolower($rateReply->ServiceType);
			if(strpos($rateReply->ServiceType, 'FEDEX') !== false){
			$fedex_shipping_method = ucwords($get_round1_shipping_method);
			} else{
			$fedex_shipping_method = 'FedEx&nbsp;'. ucwords($get_round1_shipping_method);
			}
			
                           	 $methods[] = array('id' => str_replace('_', '', $rateReply->ServiceType),
                             'title' => ucwords(str_replace('_', ' ', $fedex_shipping_method)) . $transitTime,
    ...
    } else {  //If only 1 shipping option is given
...
// end po box hack by JD
  ...
  function install() {
    ...
    tep_db_query ("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Debugging', 'MODULE_SHIPPING_FEDEX_DEBUGGING', 'off', 'Enable debugging to write XML requests and responses to file or screen', '6', '100', 'tep_cfg_select_option(array(\'off\', \'header\', \'file\'), ', now())");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, last_modified, date_added, use_function, set_function) values ('', 'Shipping Delay', 'MODULE_SHIPPING_FEDEX_WEB_SERVICES_SHIPPING_DAYS_DELAY', '1', 'How many days from when an order is placed to when you ship it (Decimals are allowed). Arrival date estimations are based on this value.', '6', '26', NULL, now(), NULL, NULL)");
    tep_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, last_modified, date_added, use_function, set_function) values ('', 'Currency Exchange Rates Conversion', 'MODULE_SHIPPING_FEDEX_WEB_SERVICES_CURRENCY', '1', 'Set your currency exchange rate here if needed. Note that this data must be a number (integer like 32 or with a floating point like 32.48) and will have to be updated by you regularly to match exchange fluctuations.', '6', '27', NULL, now(), NULL, NULL)");
  }
  ...
  function keys() {
    return array('MODULE_SHIPPING_FEDEX_WEB_SERVICES_STATUS',
                 ...
                 'MODULE_SHIPPING_FEDEX_WEB_SERVICES_SHIPPING_DAYS_DELAY',
                 'MODULE_SHIPPING_FEDEX_WEB_SERVICES_CURRENCY',
                 ...
                 'MODULE_SHIPPING_FEDEX_DEBUGGING'
                 );
  }
