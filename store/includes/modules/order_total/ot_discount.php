  ...
  class ot_discount {
    ...
    function process() {
            ...
            if (in_array($customer_id, $customers)) {
              if (!empty($check['products_id']) || !empty($check['categories_id']) || !empty($check['manufacturers_id'])) {
                ...
                if ($check['orders_total'] == 1) {
				 ...
				 // print_r($order->info['tax_groups_d']);
				  foreach($order->info['tax_groups'] as $k=>$v){
					  $scale = $order->info['subtotal']/$v;  
					  $tax_discount[$k] += (strpos($check['discount_values'], '%') === false ? ($v-($order->info['subtotal']-$discount)/$scale) : $v * str_replace('%', '', $check['discount_values']) / 100);
				  }
                }
              }
            }
      ...
      if (!empty($discount)) {
        ...
        $this->output[] = array('title' => TEXT_DISCOUNT . (!empty($order_info) ? ' (<b>' . $check['coupons_notice'] . '</b>)' : '') . ':',
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Discount Code', 'MODULE_ORDER_TOTAL_DISCOUNT_STATUS', 'true', 'Do you want to display the discount value?', '6', '1','tep_cfg_select_option(array(\'true\', \'false\'), ', now())");
