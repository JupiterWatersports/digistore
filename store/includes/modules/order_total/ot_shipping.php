...
/* One Page Checkout - END */
      ...
      if (tep_not_null($order->info['shipping_method'])) {
        ...
        //$pieces = explode(" ", $order->info['shipping_method']);
        //$first_part = implode(" ", array_splice($pieces, 0, 3));
        $first_part = preg_replace("/\([^)]+\)/","",$order->info['shipping_method'], 1);
        $this->output[] = array('title' => strpos($order->info['shipping_method'],'FedEx') !== 0 ? $order->info['shipping_method']  : $first_part ,
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_SHIPPING_SORT_ORDER', '5', 'Sort order of display.', '6', '2', now())");
