...
switch (true) {
  ...
  case ((basename($_SERVER['PHP_SELF']) === 'index.php')):
	...
	elseif ($category_depth != 'top') $category_depth='products';
       ...
       if (tep_not_null($cPath) || (isset($_GET['manufacturers_id']) && $category_depth == 'top'))
       {
          ...
          $canonical_url = StripSID(tep_href_link(FILENAME_DEFAULT, $args, 'SSL', false) );
       }
  ...
  case (basename($_SERVER['PHP_SELF']) === FILENAME_PRODUCT_REVIEWS_WRITE):
    ...
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), $id)){ 
	   ...
	   $metaQuery = tep_db_query("select products_image from " . TABLE_PRODUCTS . " where products_id ='" . (int)$_GET['products_id'] . "'");
	   ...
	   //Step 3. Extract the info from the DB
		...
		$metaQueryData = tep_db_fetch_array ($metaQuery);
			$metaProductsImg = $metaQueryData['products_image'];
...
// product_info.old.php
  case (basename($_SERVER['PHP_SELF']) === FILENAME_PRODUCT_INFO.OLD):
    if (! ReadCacheHeaderTags($header_tags_array, basename($_SERVER['PHP_SELF']), '')) {
      $header_tags_array = tep_header_tag_page(FILENAME_PRODUCT_INFO.OLD);
...
// ALL OTHER PAGES NOT DEFINED ABOVE
  ...
  //Step 7. Construct the product image
		...
		if($metaProductsImg <> ''){
			...
			$metaImg = tep_href_link(DIR_WS_IMAGES . $metaProductsImg); 
		}
...
echo '<script type="text/javascript" src="JavascriptFiles/gtag.js"></script><noscript><img height="1" width="1" style="display:none"
src="https://www.facebook.com/tr?id=234965059596038&ev=PageView&noscript=1"
/></noscript>' . "\n";
echo '<script async src="https://www.googletagmanager.com/gtag/js?id=G-G1W0J5H19E"></script>' . "\n";
echo '<script type="text/javascript" src="JavascriptFiles/ga4-analytics.js"></script>' . "\n";
