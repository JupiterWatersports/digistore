...
if(!$phpSelfFix) $PHP_SELF = (((strlen(ini_get('cgi.fix_pathinfo')) > 0) && ((bool)ini_get('cgi.fix_pathinfo') == false)) || !isset($HTTP_SERVER_VARS['SCRIPT_NAME'])) ? basename($HTTP_SERVER_VARS['PHP_SELF']) : basename($HTTP_SERVER_VARS['SCRIPT_NAME']);
...
// End get 1 free
      ...
      case 'remove_product':  if (isset($_GET['products_id'])) {
    						  ...
    						  tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters, array('module'))));
                              break;                      
...
if (!tep_session_is_registered('mobile')) {
	$mobile=0;
	include("includes/Mobile_Detect.php");
	$detect = new Mobile_Detect();
	//if ((!$detect->isBlackberrytablet())  && (!$detect->isAndroidtablet()) && (!$detect->isIpad())) {
    if ($detect->isMobile() && !$detect->isTablet()) {
	    $mobile=1;
	}
	     tep_session_register('mobile');
}

if ($_GET_['redirectCancelled'] == true) $mobile=0;
?>
