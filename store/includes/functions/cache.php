...
//  fills $var using unserialize().//*** <Cross-Sell Mod>
...
// Cache the also Xell module
  function tep_cache_xsell_products($auto_expire = false, $refresh = false) {
    global $HTTP_GET_VARS, $language, $languages_id, $currency;

    $cache_output = '';

    if (isset($HTTP_GET_VARS['products_id']) && is_numeric($HTTP_GET_VARS['products_id'])) {
      if (($refresh == true) || !read_cache($cache_output, 'xsell_products-' . $language . '-'.$currency.'.cache' . $HTTP_GET_VARS['products_id'], $auto_expire)) {
        ob_start();
        include(DIR_WS_MODULES . FILENAME_XSELL_PRODUCTS);
        $cache_output = ob_get_contents();
        ob_end_clean();
        write_cache($cache_output, 'xsell_products-' . $language . '-'.$currency.'.cache' . $HTTP_GET_VARS['products_id']);
      }
    }
