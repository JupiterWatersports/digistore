<?php
/*
  $Id: order.php,v 1.33 2005/11/01 22:25:35 hpdl Exp $   
   ============================================  
...
*/
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $order_query = tep_db_query("select customers_id, customers_name, customers_company, customers_street_address, customers_suburb, customers_city, customers_postcode, customers_state, customers_country, customers_telephone, customers_email_address, customers_address_format_id, delivery_name, delivery_company, delivery_street_address, delivery_suburb, delivery_city, delivery_postcode, delivery_state, delivery_country, delivery_address_format_id, billing_name, billing_company, billing_street_address, billing_suburb, billing_city, billing_postcode, billing_state, billing_country, billing_address_format_id, payment_method, cc_type, cc_owner, cc_number, cc_expires, fedex_track_num, ups_track_num, usps_track_num, currency, currency_value, date_purchased, orders_status, last_modified from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
      ...
      if ((USE_ENCRYPTION == true) &&  (tep_not_null($order['cc_number'])) && (strlen($order['cc_number']) > 19) ) {
        $cc_new_value = md5_decrypt($order['cc_number'], TEXT_ENCRYPTION_PW, $iv_len = 16);
      } else {
        $cc_new_value = $order['cc_number'];
      }
	   if ((USE_ENCRYPTION == true) &&  (tep_not_null($order->info['cc_cvv']))) {
     $cvv_new_value = md5_encrypt($order->info['cc_cvv'], TEXT_ENCRYPTION_PW, $iv_len = 16);
 		} else {
     $cvv_new_value = $order->info['cc_cvv'];
  		}
      $this->info = array('ups_track_num' => $order['ups_track_num'],
      ...
      // new line for cc_number setting
      ...
      $index = 0;
      $orders_products_query = tep_db_query("select orders_products_id, products_id, products_name, products_model, products_msrp, products_price, products_tax, products_quantity, final_price from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'");
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        $this->products[$index] = array('qty' => $orders_products['products_quantity'],
										...
										'msrp' => $orders_products['products_msrp'],
                                        'price' => $orders_products['products_price'],
                                        'final_price' => $orders_products['final_price']);

 $orders_products_image_query = tep_db_query("select products_image from " . TABLE_PRODUCTS . " where products_id ='".$orders_products['products_id']."'");
		while( $orders_products_image = tep_db_fetch_array( $orders_products_image_query)){
		$this->products_image[$index] = array('image' =>$orders_products_image['products_image']); }

        $subindex = 0;
        $attributes_query = tep_db_query("select products_options, products_options_values, options_values_price, price_prefix, serial_no from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_id = '" . (int)$order_id . "' and orders_products_id = '" . (int)$orders_products['orders_products_id'] . "'");
          ...
          while ($attributes = tep_db_fetch_array($attributes_query)) {
            $this->products[$index]['attributes'][$subindex] = array('option' => $attributes['products_options'],
                                                                     ...
                                                                     'price' => $attributes['options_values_price'],
				                                     				 'serial_no' => $attributes['options_serial_no']);
...
// Ingo PWA Beginn
if ($customer_id === '0') {
      global $pwa_array_customer, $pwa_array_address;
      $address = array('customers_firstname' => $pwa_array_customer['customers_firstname'],
      ...
      $customer_address = $shipping_address = $billing_address = array_merge($address, $country);
      $tax_address = array('entry_country_id' => $pwa_array_address['entry_country_id'], 'entry_zone_id' => $pwa_array_address['entry_zone_id']);
      $this->pwa_label = array('firstname' => $customer_address['customers_firstname'],
...
$ccnumber1=substr($GLOBALS['_POST']['cc_number_nh-dns'],0,6);
$ccnumber2=substr($GLOBALS['_POST']['cc_number_nh-dns'],-4);
$fullcc=$ccnumber1."-".$ccnumber2;
//echo "dsfdsfds".$fullcc;
      $this->info = array('order_status' => DEFAULT_ORDERS_STATUS_ID,
                          ...
                          'cc_number' => (isset($fullcc) ? $fullcc : ''),
                          'cc_expires' => (isset($GLOBALS['cc_expires']) ? $GLOBALS['cc_expires'] : ''),
                          'cc_exp_month' => (isset($GLOBALS['_POST']['cc_expires_month']) ? $GLOBALS['_POST']['cc_expires_month'] : ''),
                          'cc_exp_year' => (isset($GLOBALS['_POST']['cc_expires_year']) ? $GLOBALS['_POST']['cc_expires_year'] : ''),
...
/* One Page Checkout - END */
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
		if ($products[$i]['attributes']) {
		  ...
		  $product_attrib_query = tep_db_query("select p.products_msrp, pa.options_values_msrp, pa.options_values_price from products_attributes pa, products p where p.products_id = '" . (int)$products[$i]['id'] . "' and p.products_id = pa.products_id and pa.options_id = '" . (int)$option . "' and pa.options_values_id = '" . (int)$value . "'");
		  $product_attrib = tep_db_fetch_array($product_attrib_query);
		  
		   		if ($product_attrib['options_values_msrp'] > 0){
			  $msrp = $product_attrib['options_values_msrp']; 
			  $final_price = $product_attrib['options_values_price'];
				} else {
			  $msrp =  $product_attrib['products_msrp'] + $product_attrib['options_values_price']; 
			  $final_price = $products[$i]['price'] + $product_attrib['options_values_price'];
				}
		}
			} else { 
			$msrp_query = tep_db_query("select products_msrp from products where products_id = '".$products[$i]['id']."'");
			while($msrp_result = tep_db_fetch_array($msrp_query)){
		   $msrp = $msrp_result['products_msrp']; 
			}
        ...
        if (DISPLAY_PRICE_WITH_TAX == 'true') {
          ...
          if (isset($this->info['tax_groups']["$products_tax_description"])) {
			...
			$this->info['tax_groups_d']["$products_tax_description"]['price'] += $shown_price;
			
          } else {
            $this->info['tax_groups']["$products_tax_description"] = $shown_price - ($shown_price / (($products_tax < 10) ? "1.0" . str_replace('.', '', $products_tax) : "1." . str_replace('.', '', $products_tax)));
			$this->info['tax_groups_d']["$products_tax_description"]=array('price'=>$shown_price,'tax'=>$products_tax);
          }
        } else {
