...
// BOF: who's shopping modification
		if($_REQUEST["osCsid"]) {  tep_db_query("update " . TABLE_WHOS_ONLINE . " set shopping='Y' where session_id  = '" . $_REQUEST["osCsid"] ."'"); }
...
// assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
    ...
    function calculate() {
      ...
      while (list($products_id, ) = each($this->contents)) {
        $qty = $this->contents[$products_id]['qty'];
// products price
        $product_query = tep_db_query("select products_id, products_msrp, products_price, products_tax_class_id, products_weight, products_quantity from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
...
// attributes price
        if (isset($this->contents[$products_id]['attributes'])) {
          reset($this->contents[$products_id]['attributes']);
          while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
            $attribute_price_query = tep_db_query("select options_values_price, options_values_msrp, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$prid . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "'");
            $attribute_price = tep_db_fetch_array($attribute_price_query);
			
			if ($attribute_price['options_values_msrp'] > 0){
				$this->total += $qty * ($attribute_price['options_values_price'] - $products_price);
			} else {
            if ($attribute_price['price_prefix'] == '+') {
              ///$this->total += $qty * tep_add_tax($attribute_price['options_values_price'], $products_tax);/// Discount 
					// BEGIN  Discount 
							if ($discountplus_data['valuetyp'] != "price" && $discountplus_data['valuetyp'] != "endprice")
								$this->total += $qty * tep_add_tax($attribute_price['options_values_price']-(($attribute_price['options_values_price']/100)*$discountplus_data['value']), $products_tax);
							else
    ...
    function attributes_price($products_id) {
      ...
      if (isset($this->contents[$products_id]['attributes'])) {
        reset($this->contents[$products_id]['attributes']);
        while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
          $attribute_price_query = tep_db_query("select options_values_msrp, options_values_price, price_prefix from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . (int)$products_id . "' and options_id = '" . (int)$option . "' and options_values_id = '" . (int)$value . "'");
          $attribute_price = tep_db_fetch_array($attribute_price_query);
...
// BEGIN Discount 
             ...
             if(tep_db_num_rows($discountplus_query))
             {
	             ...
	             {
	             	$products_price = $products_price-(($products_price/100)*$discountplus_data['value']);
	             	$attributes_price = $attributes_price-(($attributes_price/100)*$discountplus_data['value']);
	             }
             }
	// END Discount  
          $specials_query = tep_db_query("select specials_new_products_price from " . TABLE_SPECIALS . " where products_id = '" . (int)$prid . "' and status = '1'");
          if (tep_db_num_rows($specials_query)) {
            ...
            $products_price = $specials['specials_new_products_price'];
		  	 ...
		  	 $products_options_price_query = tep_db_query("select pa.options_values_msrp, pa.options_values_price from products_attributes pa, products p where pa.products_id = '" . $prid . "' and pa.products_id = p.products_id and pa.options_values_id= '".$value."'");
		...
		$products_array[] = array('id' => $products_id,
                                    ...
                                    'final_price' => ($products_price + $attributes_price),/// Discount 
