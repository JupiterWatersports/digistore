  ...
  class headerTitle {
    ...
    function write($title = '') {
		...
		if(sizeof($cart->contents) > 0) 
			$rightButton = '<a href="' . tep_href_link(FILENAME_SHOPPING_CART) . '">' . tep_image(DIR_WS_IMAGES . "mobile/cart.png") . '</a>';
		else {
			if(isset($PCSITE))
				$url = $PCSITE;
			else {
			...
			$rightButton = '<a href="' . $url . '">' . tep_image(DIR_WS_IMAGES . "mobile/pcsite.png") . '</a>';
		}
		echo '
