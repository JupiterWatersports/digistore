 ...
 class osC_CategoryTree {
   ...
   function osC_CategoryTree($load_from_database = true) {
     ...
     global $languages_id;
         ...
         $categories_query = tep_db_query("select c.categories_id, cd.categories_name, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and cd.categories_htc_desc_tag NOT LIKE '%hidden%' order by c.parent_id, c.sort_order, cd.categories_name");
...
*/
...

	{
		...
		where prod.products_quantity > 0 and prod.products_status = '1' and prodcate.categories_id=".$categori_id." and prod.products_id=prodcate.products_id 
