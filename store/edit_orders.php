 ...
 //orders status
  ...
  if (isset($action)) {
    ...
    ////
      ...
      $query_product_id = tep_db_query("select products_barcodes_id id, products_id from " . TABLE_PRODUCTS_BARCODES . " where barcode = '" . $barcode . "'");
      if($result_product_id = tep_db_fetch_array($query_product_id))
        $pID = $result_product_id['products_id'];
      else
      {
        // Redirects to manual add
        tep_redirect(tep_href_link(FILENAME_EDIT_ORDERS, "oID=" . $oID . "&action=barcode_manual&barcode=" . $barcode));
      }
      

      // Check if the barcode has a value set for all options of the product
      $query_check_options = tep_db_query("select count(*) nb from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_BARCODES . " pb where pb.products_id = pa.products_id and pb.barcode = '" . $barcode . "' and options_id not in (select pa2.options_id from " . TABLE_PRODUCTS_ATTRIBUTES . " pa2, " . TABLE_PRODUCTS_BARCODES_TO_OPTIONS . " pbo2 where pbo2.products_barcodes_id = pb.products_barcodes_id and pa2.products_attributes_id = pbo2.products_attributes_id)");
      $result_check_options = tep_db_fetch_array($query_check_options);
      if ($result_check_options['nb'] > 0)
        tep_redirect(tep_href_link("edit_orders.php", "oID=" . $oID . "&action=barcode_manual&barcode=" . $barcode));

      
      $query_attributes_values = tep_db_query("select options_id options_id, options_values_id values_id from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_BARCODES_TO_OPTIONS . " pbo where pbo.products_attributes_id = pa.products_attributes_id and pbo.products_barcodes_id = " . $result_product_id['id']);
        ...
        if (is_array($_POST['update_products'])) {
          foreach($_POST['update_products'] as $orders_products_id => $products_details) {
			...
			if ($products_details['qty'] != $order_products['products_quantity']){
				...
				$options_query = tep_db_query("SELECT products_options_id, products_options_values_id FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " ON products_options_name = products_options LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON products_options_values_name = products_options_values WHERE orders_products_id = '" . (int)$order_products['orders_products_id'] . "'");
					while ($option = tep_db_fetch_array($options_query)) {
				//$option = tep_db_fetch_array($options_query);

				tep_db_query("update " . TABLE_PRODUCTS_ATTRIBUTES . " set options_quantity = options_quantity - " . $quantity_difference . " where products_id = '" . $order_products['products_id']. "' AND options_id = '" . (int)$option['products_options_id'] . "' AND options_values_id = '" . $option['products_options_values_id'] ."'");
				}
		   ...
		   if ( (isset($products_details['delete'])) && ($products_details['delete'] == 'on') ) {
		     //check first to see if product should be deleted
			       ...
			       if (STOCK_LIMITED == 'true'){
					...
					$options_query = tep_db_query("SELECT products_options_id, products_options_values_id FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " ON products_options_name = products_options LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON products_options_values_name = products_options_values WHERE orders_products_id = '" . (int)$order_products['products_id'] . "'");
					$option = tep_db_fetch_array($options_query);

					tep_db_query("update " . TABLE_PRODUCTS_ATTRIBUTES . " set options_quantity = options_quantity + " . $products_details["qty"] . " where products_id = '" . (int)$order_products['products_id']. "' AND options_id = '" . (int)$option['products_options_id'] . "' AND options_values_id = '" . $option['products_options_values_id'] ."'");
          ...
          <ul>
			 ...
			 <?php if (ORDER_EDITOR_USE_AJAX == 'true') { ?>
              ...
              document.write("<li><a href=\"javascript:newOrderEmail()\"><img src=\"includes/languages/<?php echo $language; ?>/images/buttons/button_new_order_email.gif\" border=\"0\" alt=\"<?php echo IMAGE_NEW_ORDER_EMAIL; ?>\" title=\"<?php echo IMAGE_NEW_ORDER_EMAIL; ?>\" ></a></li>");
	           //--></script>
			   <?php } ?>
		    <li><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $_GET['oID']. '&action=email') . '" TARGET="_blank">' . tep_image_button('button_send.gif', IMAGE_ORDERS_INVOICE) . '</a>'; ?></li>	 
...
?>
 ...
 <div id="signature-block">
			 <table width="100%" border="0" cellspacing="0" cellpadding="2" style="border: 1px solid #C9C9C9;">
  <tr class="dataTableHeadingRow"> 
	<td colspan="4" class="dataTableHeadingContent" valign="top"><?php echo 'Signature'; ?></td>
  </tr>
  <tr class="dataTableRow"> 
	<td colspan="4" valign="top" class="dataTableContent">
		...
		<?php }else{ ?>
			...
			<style>
				...
				#signature{ width:500px; height:160px; border:1px solid #eee;}
				
			</style> 
			<div id="signature"></div>
			<div class="controls">
				<input type="button" value="I Agree" onclick="submitSign()"/>
...
?>
		...
		<div id="totalsBlock">
			...
			<div id="add-products-block">
				  ...
				  <br>*/ ?><input type="hidden" name="subaction" value="">
				  
				  <iframe src="<?php echo tep_href_link(FILENAME_ORDERS_EDIT_ADD_PRODUCT, 'oID=' . $_GET['oID'] . '&step=1'); ?>" class="autoHeight" frameborder="0" style="width:100%;"></iframe>
				  
				  </div>
                  
                    <!-- order_totals bof //-->
              <div id="mobile-order-totals-block">
