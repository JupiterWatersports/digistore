...
if (EMAIL_USE_HTML == 'true') {$email_text;} 
else {
if (ACCOUNT_GENDER == 'true') {
       if ($HTTP_POST_VARS['gender'] == 'm') {
         $email_text = EMAILGREET_MR;
       } else { $email_text = EMAILGREET_MS;
       }
    } else { $email_text = EMAILGREET_NONE;
    }
...
// EOF: MOD - Separate Pricing Per Customer: alert shop owner of account created by a company
      ...
      else { tep_redirect(tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL'));
    }
   }
  }
...
<?php    
  if ($messageStack->size('checkout_guest') > 0) {
	 echo $messageStack->output('checkout_guest');
 	 }
?>
...
<div class="spacer-forms"><label>Street Address 1</label>
...
<div class="spacer-forms"><label>Street Address 2</label>
...
<div class="state"><label>State</label><br />
<div id="states">
<?php
// +Country-State Selector
...
<div><?php echo CATEGORY_OPTIONS; ?></div>
...
<?php
  if (strstr($PHP_SELF,'checkout_guest') &&  ACCOUNT_CREATE_VALIDATION == 'true') include(DIR_WS_MODULES . FILENAME_DISPLAY_VALIDATION); ?>
<!-- // EOF Anti Robot Registration v3.0-->
...
?>        
<button style="float:left; float: left; margin-top: 40px;" class="button-blue-small">Continue</button></div>
</form>
</div>
