...
if (tep_db_num_rows($check_att_query) < 1) {
 ...
 if ($products[$i]['special_order'] !== '1'){
    ?>
     ...
     <?php  }}} if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
	reset($products[$i]['attributes']);
	while (list($option, $value) = each($products[$i]['attributes'])) {
	$products_id = $products[$i][$option]['products_id'];
  ...
  echo  '<div class="cart-item-image"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '">' . tep_image(DIR_WS_IMAGES . $product_image, $products[$i]['name'], 150, 150). '</a></div>' .
                       '<div class="cart-details-container">'.
					   '<div class="cart-item-details">'.'<a class="cart-item-name" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products_id) . '">' . $products[$i]['name'] . '</a>' 
...
<?php } 	if($products[$i][$option]['options_values_msrp'] > 0){
