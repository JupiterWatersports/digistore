...
<div style="display:inline;float:left"><form method="POST" id="'.$url_id.'">
<input type="hidden" name="step" value="3">';
	$field .= '<input type="hidden" name="add_product_products_id" id="add_product_products_id" value="'. $url_id.'">
	<input type="hidden" name="product_search" id="product_search" value="'.$url_title.'">';
	...
	$check_attributes_stock_yeah = tep_db_query("select options_quantity from products_attributes where products_id = '".$url_id."' and products_attributes_id = '".$productsattributes."'");
	$check_attributes_stock = tep_db_fetch_array($check_attributes_stock_yeah);
		
		if($check_attributes_stock['options_quantity'] < '1'){
		$out_of_stock = '****';			
		}	
		
	   $field .='<div class="url-holder">';
	  $field .="<script>
$('#submitform".$productsattributes."').click(function()
...
$field .= '<input type="hidden" name="add_product_attributes_id[' . $value . ']" id="products_attribute_id" value="'. $productsattributes . '">';
$field .= '<input type="hidden" name="search" value="1">';
$field .= '<input type="hidden" name="product_search" id="product_search" value="' .$products_options_name .' '. tep_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '"';
        $field .= 'readonly style="width:400px;  background:#fff; border:none; align:left;">
		<div style="font-size:12px !important; padding-left:20px; width:100%; cursor:pointer;" id="submitform'. $productsattributes . '">'.$out_of_stock.'&nbsp;'.$products_options_name .' '. tep_output_string($values[$i]['text']).'</div></form></div></div>';
...
while ($row = mysqli_fetch_assoc($sqlresult)){
  ...
  $url_title = $addtitional . htmlspecialchars($row['products_name']);
  ...
  else
  {
    ...
    //$addlink = '<a href="' . $url_url . '&action=buy_now&sort=2a">' . BUY_NOW . '</a>'; //Add To Cart Button
     $products_attributes_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_ATTRIBUTES . " patrib where patrib.products_id='" . $url_id . "' and patrib.options_id = popt.products_options_id and popt.language_id = '" . (int)$languages_id . "'");
...
<div style="display:inline;float:left; width:100%;">
 <form method="POST" id="products<?php echo $url_id; ?>"><input type="hidden" name="step" value="3"><input type="hidden" name="add_product_products_id" id="add_product_products_id" value="<?php echo $url_id; ?>"><input type="hidden" name="search" value="1">
...
<div class="url-holder">
<div style="display:inline;float:left; width:100%;">
 <form method="POST" id="products<?php echo $url_id; ?>"><input type="hidden" name="step" value="5">
...
$('#submitform<?php echo $url_id; ?>').click(function()
 {	var data = $('#products<?php echo $url_id; ?>').serialize();
  $.ajax({
  type : 'POST',
