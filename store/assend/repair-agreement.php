...
if ($action == 'update_signature') {
  ...
  tep_db_perform('repair_table', $sql_data_array);
...
<h1 style="text-align:center; text-transform:uppercase;">Repair Agreement</h1>
...
<div class="form-group" style="margin-bottom:20px;">Effective Date: <b><?php echo $date; ?></b> </div>
<div class="form-group" style="margin-bottom:20px;">Between <b>Jupiter Kiteboarding</b>, further referred to as "[Seller]", </div>

<div class="form-group" style="margin-bottom:20px;">And <b><?php echo $_POST['update_customer_name']; ?></b>, further referred to as "[Customer]" </br></br>
<b>Both parties agree to the following terms:</b>
</div>
<ol>
<li>Seller will charge Customer an up-front $75.00 deposit fee to cover the initial inspection and testing of
equipment.
</br><ol type="a">
    <li>The $75 deposit fee will be applied to the first hour of inspection and testing if Seller’s recommended repairs are accepted and completed.</li>
    <li>Seller’s hourly repair rate is then $75/ hr. for each additional hour (plus all necessary parts).</li>
    <li>The $75 deposit fee will be retained by Seller if Seller’s recommended repairs are not accepted, and
the equipment will then be returned non-repaired/ as-is.</li>
</ol></br>
<li>After inspection, Seller will contact Customer (via primary contact method listed below) with the total cost and time to repair. If accepted, Seller will then perform the applicable repairs as quoted.</li></br>
<li>Upon repair completion, Seller will contact Customer (via primary contact method listed below) to pick up
the equipment.</li></br>
<li>If repairs are not accepted, Seller will then ask Customer to pick up the non-repaired equipment.</li>
</ol>

<b>NOTE:</b> Seller will hold the equipment (repaired or non-repaired) at Seller’s primary facility for 2 weeks. Seller will continue to contact Customer during this 2-week period to remind Customer to pick it up. If not picked up after this 2-week time period, Seller will then move the equipment to a remote secured storage facility. Customer will then need to advise Seller 2 days in advance of when they will pick up your equipment so Seller can retrieve it.
</br></br>
If Customer’s equipment is not picked up within 60 days after initial contact, Seller will consider it abandoned
and will then dispose of it however Seller deems necessary.</br></br>
          </ol>
<div>

<b>Applicable Law</b>
</br></br>
<div>This contract shall be governed by the laws of the State of Florida and any/ all applicable Federal Laws.</div>
</br></br>
<div>I HAVE READ AND AGREE TO THIS AGREEMENT IN ITS ENTIRETY.</div>
...
<div>
<b>Customer Name:</b> <?php echo $_POST['update_customer_name']; ?>    Date: <?php echo $date; ?>
</div>

<div>
<br/>
<br/>
Primary Contact Method <b>Cell:</b>   <?php echo $_POST['update_customer_telephone']; ?>  <b>or Email</b> <?php echo $_POST['update_customer_email_address'];?>
</div>

</div>

<?php $check_sig_query = tep_db_query("SELECT * FROM repair_table WHERE order_id = '".$_GET['oID']."'");
...
overlay.removeClass("show-overlay");
	...
	function sendContract(){
		...
		$.ajax({
  			type : 'POST',
  			url  : 'repair-agreement.php?oID='+<?php echo $_GET['oID']; ?>+'',
  			data : data,
  			success :  function(data) {
                $("#repair-container").html(data);
