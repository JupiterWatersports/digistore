...
$default_payment_methods = "Visa,MasterCard,WireTransfer"; // Acceptable values: cash, check, GoogleCheckout, Visa, MasterCard, AMEX, Discover, wiretransfer
...
$catTempDes = array();
$catTempPar = array();
$processCat = tep_db_query( $catInfo )or die( $FunctionName . ": SQL error " . mysql_error() . "| catInfo = " . htmlentities($catInfo) );
...
if($optional_sec =='true')
{
	...
	if($feed_quantity =='true')
  	...
  	$output .= $Separator."brand".$Separator."condition"; 
}
$output .= "\n";
...
if($convertCur)
{
	...
	$result3=tep_db_query( $sql3 )or die( $FunctionName . ": SQL error " . mysql_error() . "| sql3 = " . htmlentities($sql3) );
...
while( $row = mysql_fetch_object( $result ) )
{
	...
	if( $row->prodStatus == 1 || ($advertise_inactive_with_stock =='true' && $quantity == 1) )
	{
		...
		$descriptionOutput = preg_replace($_strip_search, $_strip_replace, strip_tags( strtr($row->description, $_cleaner_array) ) );

		$output .= preg_replace($_strip_search, $_strip_replace, strip_tags( strtr($row->name, $_cleaner_array) ) ) . $Separator . 
		str_replace('/"/','',$descriptionOutput) . $Separator .
		$feed_exp_date . $Separator .
		str_replace(',', '',$row->price) . $Separator .
