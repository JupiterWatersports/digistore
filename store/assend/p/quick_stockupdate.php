...
/* extra fields added begin */
			if($opt['value_upc'] != ''){
				$sql = "UPDATE products_attributes SET options_upc = '".$opt['value_upc']."' WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']."";
				tep_db_query($sql );
			}
...
<!-- extra fields added begin-->
<td align="left" class="smallText">&nbsp;UPC:&nbsp;<input name="stock_update[opt][<?php echo $products_options['products_options_values_id'];?>][<?php echo $products_options['options_id'].'-'.$results['products_id'];?>][value_upc]" type="text" class="prodfield_<?php echo  $results['products_id'] ?>"   value="<?php echo $products_options['options_upc']; ?>" size="6"></td>
