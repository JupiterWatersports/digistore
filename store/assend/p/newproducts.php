...
<!--<div class="pl products-new">-->
	...
	<!--button--> 
	<div class="pl-button">	
	<?php echo '<form name="buy_now_' . $products_new['products_id'] . '" method="post" action="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now', 'NONSSL') . '"><button class="cssButton buynow" style="border:none;"><input type="hidden" name="products_id" value="' . $products_new['products_id'] . '" />' . 'Buy Now' . '</button>' .'</form>'; ?>
 	</div>
...
<div class="col-sm-6"><?php echo '' . ' ' . $products_new_split->display_links(MAX_DISPLAY_PAGE_LINKS, tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?>
</div>
</div>
