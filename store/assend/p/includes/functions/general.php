...
// display contact details on print product page
  function tep_display_print(){
  echo "<br />&nbsp;<B>" . STORE_NAME . "<br />&nbsp;" . STORE_NAME_ADDRESS . "&nbsp; </B>";
  }
////
...
$get_id_vars = str_replace(strstr($_GET['products_id'], '{'), '', $_GET['products_id']); // Remove attributes
...
if (!function_exists("tep_mobile_button")) {
...
if (!function_exists("tep_mobile_selection")) { 
  function tep_mobile_selection($path, $elements) {
  	...
  	$ret .= ($path) ?  '<a href="' . $path .'">' . tep_image(DIR_WS_IMAGES . 'arrow_select.png') . '</a>' : '<input type="image" src="' .DIR_WS_IMAGES . 'arrow_select.png' . '" border="0" alt="">';
	return $ret;
  	
  }
}
if (!function_exists("tep_print")) { 
...
function oe_get_country_id($country_name) {
  ...
  function oe_get_zone_id($country_id, $zone_name) {
    $zone_id_query = tep_db_query("select zone_id from " . TABLE_ZONES . " where zone_country_id = '" . (int)$country_id . "' and (zone_name = '" . tep_db_input($zone_name) . "' OR zone_code = '" . tep_db_input($zone_name) . "')");
