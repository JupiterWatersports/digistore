     ...
     // $customer_orders_string = '<table class="infobox">';
      ...
      while ($products = tep_db_fetch_array($products_query)) {
        $customer_orders_string .= '  ' .
                                   '    <div class="dotted-line"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products['products_id']) . '">' . $products['products_name'] . '</a>' .
                                   '    <a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=cust_order&pid=' . $products['products_id']) . '">' . tep_image(DIR_WS_ICONS . 'cart.gif','infobox cart','','',ICON_CART,'infobox-cart') . '</a></div>' .
                                   '  ';
      }
