...
// 1.2 Test for presence of status field for compatibility with older versions
  ...
  $query = "select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . (int)$category_links[$i] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' order by sort_order, cd.categories_name";
...
. $subcategories['categories_id'];
                ...
                $text_subcategories .= '<br /><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new_sub, 'NONSSL') . '">' . $subcategories['categories_name'] . '</
a>';
                } else {
		 $text_subcategories .= ' ';
		}// if
    ...
    echo '<div style="width:47%; float:left; border: solid 1px #cecece; margin: 7px; font-size:11px;">
<div style="background:#E1E1E1; padding: 5px 0 0 10px; height:20px;"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new, 'NONSSL') . '">'.$categories['categories_name'].'</a></div>
<div style="float:left; width:65%; font-size:11px; padding-left:5px; text-align:left; padding-bottom:5px;">' . $text_subcategories.'</div>
<div style="vertical-align:top; width:30%; float:left; text-align:right; padding-bottom:5px;"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new, 'NONSSL') . '">' .  tep_image(DIR_WS_IMAGES . $categories['categories_image'], $categories['categories_name'], '100', '100') . '</a></div></div></div>';
    $col ++;
    if ($col > 1) {
      $col = 0;
      $row ++;
