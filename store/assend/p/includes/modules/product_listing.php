...
$url2 = $cat. $categories_stringy. $brands_stringy .$filter_stringy . $prices_stringy;	  
?>
...
echo ''.tep_draw_pull_down_menu('filter_id', $options, (isset($HTTP_GET_VARS['filter_id']) ? $HTTP_GET_VARS['filter_id'] : ''), 'onchange="this.form.submit()" class="form-control" style="width:150px;" id="sortBY" ');
	...
	$selected = '4';
	echo ''.tep_draw_pull_down_menu('', $options, $selected, 'onchange="this.form.submit()" class="form-control" style="width:150px;" id="sortBY" ');
        echo tep_hide_session_id() . '</form></div>' . "\n";
}
...
echo '<label><b>Filters:</b> </label>';
	...
	if(isset($_GET['category'])){
		...
		echo'<div class="remove-filter form-group"><span class="" style="margin-bottom:10px;"><a href="'.$dest_url.'"><i class="fa fa-times" aria-hidden="true" style="margin-right:5px; font-size:16px; color:#D9534F;"></i></a>'.$Cvalue.'</span></div>';
		}
	}
	...
	if(isset($_GET['brand'])){
		...
		$brand_trim_url = $url_complete['path'] .'?'.$brand_query;		
		  ...
		  echo'<div class="remove-filter form-group"><span class="" style="margin-bottom:10px;"><a href="'.$brand_trim_url.'"><i class="fa fa-times" aria-hidden="true" style="margin-right:5px; font-size:16px; color:#D9534F;"></i></a>'.$Bvalue.'</span></div>';
		}
	}
	...
	if(isset($_GET['maxprice']) && tep_not_null($_GET['maxprice'])){
		...
		$Rvalue = '$'.$_GET['maxprice'];
		
		parse_str($url_complete['query'], $Rquery); //grab the query part
		unset($Rquery['maxprice']);   //remove a parameter from query
		$range_query = http_build_query($Rquery); //rebuild new query
		$range_trim_url = $url_complete['path'] .'?'.$range_query;
			
		  echo'<div class="remove-filter form-group"><span class="" style="margin-bottom:10px;"><a href="'.$range_trim_url.'"><i class="fa fa-times" aria-hidden="true" style="margin-right:5px; font-size:16px; color:#D9534F;"></i></a>'.$Rvalue.'</span></div>';
	...
	if(isset($_GET['gender'])){
		...
		$gender_trim_url = $url_complete['path'] .'?'.$gender_query;
		  ...
		  echo'<div class="remove-filter form-group"><span class="" style="margin-bottom:10px;"><a href="'.$gender_trim_url.'"><i class="fa fa-times" aria-hidden="true" style="margin-right:5px; font-size:16px; color:#D9534F;"></i></a>'.$genvalue.'</span></div>';
		}
	}		
	
	echo '<a class="form-group" href="'.$url_complete['path'].'?catt='.$_GET['catt'].'" style="margin-left:15px; color:#2A9DD4">Remove All</a></div>';
...
/*		$list_box_contents[$cur_row][] = array('align' => $lc_align,
	  ...
	  $srcset_image = ''.DIR_WS_IMAGES . $listing['products_image'].' 1x, '.DIR_WS_IMAGES . $listing['products_image_hd'].' 2x';	}
	  else { $srcset_image = '';
	  }
		
echo '<div id="product-block" class="col-sm-4" style="text-align:center;">
		<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id']) . '">' .'<div class="col-xs-4 listingimg">
		<img src="'.DIR_WS_IMAGES . $listing['products_image'].'" srcset="'.$srcset_image.'" width="'.SMALL_IMAGE_WIDTH.'" height="'.SMALL_IMAGE_HEIGHT.'" alt="'.$listing['products_name'].'"></img></div>'.
