  ...
  class ot_tax {
    ...
    function ot_tax() {
      ...
      $this->output = array();
       ...
       function process() {
      global $order, $currencies, $tax_discount; // Discount Code 2.9

      reset($order->info['tax_groups']);
      while (list($key, $value) = each($order->info['tax_groups'])) {
        if (!empty($tax_discount[$key])) $value -= $tax_discount[$key]; // Discount Code 2.9
        if ($value > 0) {
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_TAX_SORT_ORDER', '4', 'Sort order of display.', '6', '2', now())");
