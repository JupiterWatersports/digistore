...
// Get manufacturer box from db or cache  
  ...
  $sts->restart_capture ('headersearchbox', 'box'); // Get header search box

  require(DIR_WS_BOXES . 'coupon.php');
  $sts->restart_capture ('couponbox', 'box'); // Get coupon box
...
// get language box for templates
    include(DIR_WS_BOXES . 'sts_header_languages.php');
    $sts->restart_capture ('stslanguagebox', 'box');
  ...
  if (basename($PHP_SELF) != FILENAME_PRODUCT_REVIEWS_INFO) {
   ...
   if (basename ($PHP_SELF) != FILENAME_RECENTLY_VIEWED) {
    ...
    $sts->restart_capture ('recentbox', 'box');  // Get the reviews box
  } else {
    $sts->template['recentbox']='';
  }	

// get product print button
    include(DIR_WS_BOXES . 'sts_product_print.php');
    $sts->restart_capture ('stsproductprintbox', 'box');
?>