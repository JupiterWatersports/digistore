...
if ( false !== strpos( strtolower( $_SERVER["SCRIPT_NAME"] ), osCSec_selfchk() ) )
    senda404Header();
...
if ( file_exists( $oscfilepath ) ) {
    ...
    # if osc.php missing
    $osC_Sec = new osC_Sec();
    $osC_Sec->Sentry( $timestampOffset = 0, $nonGETPOSTReqs = 0, $spiderBlock = 0, $banipaddress = 0, $useIPTRAP =
        0, $ipTrapBlocked = "", $emailenabled = 0, $youremail = "", $fromemail = "", $disable_tellafriend = null );
}
...
class osC_Sec {
    ...
    function Sentry( $timestampOffset = 0, $nonGETPOSTReqs = 0, $spiderBlock = 0, $banipaddress = 0, $useIPTRAP =
        0, $ipTrapBlocked = "", $emailenabled = 0, $youremail = "", $fromemail = "", $disable_tellafriend = null ) {
        ...
        $this->_currentVersion = "5.0.8";
        $spiderBlock = ( bool )$this->fINT( $spiderBlock );
        $this->rprefix = "osC_Sec detected ";

        $this->_disable_tellafriend = 0;
        if ( isset( $disable_tellafriend ) )
            $this->_disable_tellafriend = ( bool )$this->fINT( $disable_tellafriend );
    ...
    function osCEmailer( $r, $fullreport = true ) {
        ...
        if ( ( false !== strpos( $this->_youremail, "youremail@yourdomain.com" ) ) || empty( $this->
            _youremail ) )
            return;

        # send the notification
        if ( ( ( false !== $this->_banipaddress ) || ( false !== $this->_useIPTRAP ) ) && ( false !== $this->
            _emailenabled ) ) {
            global $PHP_SELF;
            ...
            if ( ! isset( $this->_timestampOffset ) )
                $this->_timestampOffset = 0;
            $timestamp = gmdate( "D, d M Y H:i:s", time() + ( $this->_timestampOffset * 3600 ) );
            $to = $this->_youremail;
            $subject = $this->_httphost . " " . ( substr( $r, 0, 60 ) ) . "...";
            $body = "This IP [ " . $this->realIP . " ] has been " . $banAction . " on the http://" . $this->
                _httphost . " website by osC_Sec.php version " . $this->_currentVersion . "\n\nREASON FOR BAN: " . $r .
                "\n\nTime of ban: " . $timestamp . "\n";
            $body .= "\n.------------[ ALL \$_GET VARIABLES ]-------------\n#\n";
            if ( ! empty( $_GET ) ) {
            ...
            $body .= "\n\nResolve IP address: http://en.utrace.de/?query=" . $this->realIP . "\n";
            $body .= "Search Project Honeypot: http://www.projecthoneypot.org/ip_" . $this->realIP . "\n\n";
     ...
     */
    function setGlobals() {
        if ( @phpversion() >= "5.0.0" && ( ! ini_get( "register_long_arrays" ) || @ini_get( "register_long_arrays" ) ==
            "0" || strtolower( @ini_get( "register_long_arrays" ) ) == "off" ) ) {
            global $HTTP_POST_VARS, $HTTP_GET_VARS, $HTTP_SERVER_VARS, $HTTP_COOKIE_VARS, $HTTP_ENV_VARS, $HTTP_POST_FILES,
                $HTTP_SESSION_VARS;
    ...
    function senda403Header() {
        $header = array( "HTTP/1.1 403 Access Denied", "Status: 403 Access Denied by osC_Sec",
            "Content-Length: 0" );
    ...
    function tinoRahui( $r ) {
        ...
        if ( ( false !== $this->_banipaddress ) && ( false !== $this->hCoreFileChk( $this->_htaccessfile ) ) ) {
            ...
            # add ip to htaccess
            $this->htaccessbanip( $this->realIP );
    ...
    function osCAdminLoginBypass() {
        ...
        if ( false !== strpos( $this->getREQUEST_URI(), ".php/login" ) ) {
            $r = $this->rprefix . "an attempt to exploit the admin login bypass exploit. ";
    ...
    function disable_tellafriend() {
        if ( false === $this->_disable_tellafriend || empty( $this->_disable_tellafriend ) || 0 == $this->
            _disable_tellafriend )
            return;
        global $PHP_SELF;
        $errlevel = ini_get( 'error_reporting' );
        if ( false === strpos( $PHP_SELF, "tell_a_friend.php" ) ) {
            return;
        } elseif ( false !== strpos( $PHP_SELF, "tell_a_friend.php" ) && ( "POST" == $_SERVER["REQUEST_METHOD"] ) &&
        isset( $_GET["products_id"] ) ) {
            $r = $this->rprefix . "an attempt to send spam via the tell_a_friend.php file. ";
            $this->banChecker( $r, true );
            return;
        } elseif ( false !== strpos( $PHP_SELF, "tell_a_friend.php" ) && isset( $_GET["products_id"] ) ) {
            error_reporting( 0 );
     ...
     */
    function dbShield() {
        if ( false === $this->byPass() || empty( $_SERVER["QUERY_STRING"] ) )
            return;
        $_query = stripslashes( $_SERVER["QUERY_STRING"] );
        if ( false !== $this->injectionMatch( strtolower( $_query ) ) ) {
            $r = $this->rprefix . "a database injection attempt: [ " . $_query . " ]. ";
            $this->banChecker( $r, true );
            return;
        }
        if ( ( strlen( $_SERVER["HTTP_USER_AGENT"] ) > 1 ) && false !== $this->injectionMatch( $_SERVER["HTTP_USER_AGENT"] ) ) {
            $r = $this->rprefix . "\$_SERVER[ \"HTTP_USER_AGENT\" ] which contains a database injection attempt: [ " .
     ...
     */
    function postShield() {
        if ( ( ! isset( $_POST ) ) || ( "POST" !== $_SERVER["REQUEST_METHOD"] ) || ( false === $this->
            byPass() ) )
            return;
        ...
        while ( $i < count( $pnodes ) ) {
            foreach ( $postvar_blacklist as $blacklisted ) {
                ...
                if ( ( is_string( $pnodes[$i] ) ) && ( strlen( $pnodes[$i] ) > 0 ) ) {
                    if ( ( false !== strpos( $pnodes[$i], $blacklisted ) ) || ( false !== strpos( urldecode( $pnodes[$i] ),
                        urldecode( $blacklisted ) ) ) ) {
                        $r = $this->rprefix . "an attempt to post malicious content: " . htmlspecialchars( stripslashes( $blacklisted ) ) .
                            ". ";
                        $this->banChecker( $r, true );
                        return;
                    }
    ...
    function getShield() {
        if ( ( false === $this->byPass() ) || empty( $_SERVER["QUERY_STRING"] ) )
            return;
        $reqvar_blacklist = array( "php/login", "eval(base64_decode", "asc%3Deval", "eval(\$_",
            "fromCharCode", "; base64", "base64,", "onerror=alert(", "tep_db_query", "../cmd", "rush=",
            "pwtoken_get", "EXTRACTVALUE(", "phpinfo(", "php_uname", "%3Cform", "passthru(", "sha1(", "sha2(",
            "<%3Fphp", "}%00.", "/iframe", "\$_GET", "ob_starting", ") limit", "%20and%201=1", "document.cookie",
            "document.write", "onload%3d", "document.location", "window.location", "location.replace\(",
            "@@datadir", "/FRAMESET", "<a href=", "[url=http://", "[/url]", "[link=http://", "[/link]",
            "_START_", "%000", "onunload%3d", "PHP_SELF", "shell_exec", "\$_SERVER", "substr(", "\$_POST", "\$_SESSION",
            "\$_REQUEST", "\$_ENV", "GLOBALS[", "\$HTTP_", ".php/admin", "mosConfig_", "%3C@replace(", "hex_ent",
            "inurl:", "replace(", "/iframe>", "return%20clk", "login.php?action=backupnow", "php/password_for",
            "unhex(", "error_reporting(", "HTTP_CMD", "=alert(", "version()", "localhost", "})%3B", "Set-Cookie",
            "%27%a0%6f%72%a0%31%3d%31", "%bf%5c%27", "%bf%27", "%ef%bb%bf", "%8c%5c", "%a3%27", "%20regexp%20",
            "JHs=", "HTTP/1.", "{\$_", "PRINT @@variable", "xp_cmdshell", "xp_availablemedia",
            "*(|(objectclass=", "||UTL_HTTP.REQUEST", "<script>" );
        ...
        if ( ( false !== ( bool )preg_match( "/onmouse(?:down|over)/i", $v ) ) && ( 2 < ( int )
            preg_match_all( "/c(?:path|tthis|t\(this)|http:|(?:forgotte|admi)n|sqlpatch|,,|ftp:|(?:aler|promp)t/i",
            $v, $matches ) ) ) {
            $injectattempt = true;
        } elseif ( ( ( false !== strpos( $v, "ftp:" ) ) && ( substr_count( $v, "ftp" ) > 1 ) ) && ( 2 < ( int )
        preg_match_all( "/@|\/\/|:/i", $v, $matches ) ) ) {
            $injectattempt = true;
        } elseif ( ( "POST" == $_SERVER["REQUEST_METHOD"] ) && ( false !== ( bool )preg_match( "/(?:showimg|cookie|cookies)=/i",
        $v ) ) ) {
            $injectattempt = true;
        ...
        } elseif ( ( false !== ( bool )preg_match( "/%0D%0A/i", $thenode ) ) && ( false !== strpos( $thenode,
        "utf-7" ) ) ) {
            $injectattempt = true;
        } elseif ( false !== ( bool )preg_match( "/php:\/\/filter|convert.base64-(?:encode|decode)|zlib.(?:inflate|deflate)/i",
        $v ) || false !== ( bool )preg_match( "/data:\/\/filter|text\/plain|http:\/\/(?:127.0.0.1|localhost)/i",
            $v ) ) {
            $injectattempt = true;
        }
        if ( false !== ( bool )$injectattempt ) {
            $r = $this->rprefix . "an attempt to read or include unauthorized file content. ";
            $this->banChecker( $r, true );
            return;
        }
        foreach ( $reqvar_blacklist as $blacklisted ) {
            ...
            if ( ( false !== strpos( $thenode, $blacklisted ) ) || ( false !== strpos( $this->url_decoder( urldecode
                ( $thenode ) ), $this->url_decoder( $blacklisted ) ) ) ) {
                $r = $this->rprefix . "a banned blacklist request_uri item: " . htmlspecialchars( $blacklisted ) .
                    ". ";
                $this->banChecker( $r, true );
                return;
            }
            # simple check of the http_user_agent against the blacklist irregardless of REQUEST_METHOD
            if ( ( false !== strpos( strtolower( $_SERVER["HTTP_USER_AGENT"] ), $blacklisted ) ) || ( false !==
                strpos( $this->url_decoder( $_SERVER["HTTP_USER_AGENT"] ), $this->url_decoder( $blacklisted ) ) ) ) {
                $r = $this->rprefix . "\$_SERVER[ \"HTTP_USER_AGENT\" ] which contains a banned string: " .
                    htmlspecialchars( $blacklisted ) . ". ";
                $this->banChecker( $r, true );
                return;
            }
        }
        # check each part of the $_SERVER[ "QUERY_STRING" ] against the blacklist
            ...
            while ( $i < count( $gnodes ) ) {
                if ( false !== is_string( $gnodes[$i] ) ) {
                    if ( false !== strpos( $gnodes[$i], "=" ) ) {
                        $findeq = strpos( $gnodes[$i], "=" );
                        $gvar = strtolower( substr( $gnodes[$i], 0, $findeq ) );
                        $gval = substr( $gnodes[$i], ( $findeq + 1 ), strlen( $gnodes[$i] ) - $findeq );
                    } else {
                        $gvar = $gnodes[$i];
                        $gval = "";
                    }
                    ...
                    if ( ! empty( $gval ) ) {
                        if ( ( false !== strpos( $gval, "\\x" ) ) || ( false !== strpos( $gval, "%" ) ) ) {
                            $gvalhexcheck = $this->hextoascii( $gval );
                        } else
                            $gvalhexcheck = null;
                        if ( false !== $this->issetStrlen( $gvalhexcheck ) ) {
                            $gval64 = $gvalhexcheck;
                            $gvalhexcheck = strtolower( $gvalhexcheck );
                            $gurldecvalhexcheck = $this->url_decoder( urldecode( $gvalhexcheck ) );
                        } else
                            $gval64 = $gval;
                        $gval64 = strtolower( base64_decode( $this->url_decoder( $gval64 ) ) );
                        $gfval = strtolower( $this->cleanString( 2, $gval ) );
                    }
                    foreach ( $reqvar_blacklist as $blacklisted ) {
                        $blacklisted = strtolower( $this->url_decoder( $blacklisted ) );
                        if ( ( false !== strpos( $gvar, $blacklisted ) ) || ( false !== strpos( $this->url_decoder( urldecode
                            ( $gvar ) ), $this->url_decoder( $blacklisted ) ) ) ) {
                            $r = $this->rprefix . "a getShield() listed banned query_string variable: " . htmlspecialchars( $blacklisted ) .
                                ". ";
                            $this->banChecker( $r, true );
                            return;
                        }
                        if ( false !== $this->issetStrlen( $gfvar ) && ( false !== strpos( $gfvar, $blacklisted ) ) || ( false
                            !== strpos( $this->url_decoder( urldecode( $gfvar ) ), $this->url_decoder( $blacklisted ) ) ) ) {
                            $r = $this->rprefix . "a getShield() listed banned query_string filtered variable: " .
                                htmlspecialchars( $blacklisted ) . ". ";
                            $this->banChecker( $r, true );
                            return;
                        }
                        if ( ( false !== $this->issetStrlen( $gvalhexcheck ) || false !== $this->issetStrlen( $gurldecvalhexcheck ) ) &&
                            ( ( false !== strpos( $gvalhexcheck, $blacklisted ) ) || ( false !== strpos( $gurldecvalhexcheck, $this->
                            url_decoder( $blacklisted ) ) ) ) ) {
                            $r = $this->rprefix . "a banned hexcode query_string value: " . htmlspecialchars( $blacklisted ) .
                                ". ";
                            $this->banChecker( $r, true );
                            return;
                        }
                        if ( false !== $this->issetStrlen( $gvalhexcheck ) && false !== $this->injectionMatch( $gvalhexcheck ) ) {
                            $r = $this->rprefix . "a hexcode database injection attempt: [ " . $gvalhexcheck . " ]. ";
                            $this->banChecker( $r, true );
                            return;
                        }
                        if ( false !== $this->issetStrlen( $gval64 ) && ( false !== strpos( $gval64, $blacklisted ) ) ) {
                            $r = $this->rprefix . "a base64 encoded blacklisted query_string value: " . htmlspecialchars( $blacklisted ) .
                                ". ";
                            $this->banChecker( $r, true );
                            return;
                        }
                        if ( false !== $this->issetStrlen( $gfval ) && ( ( false !== strpos( $gfval, $blacklisted ) ) || ( false
                            !== strpos( $this->url_decoder( urldecode( $gfval ) ), $this->url_decoder( $blacklisted ) ) ) ) ) {
                            $r = $this->rprefix . "a blacklist query_string filtered value: " . htmlspecialchars( $blacklisted ) .
                                ". ";
                            $this->banChecker( $r, true );
                            return;
                        }
    ...
    function _REQUEST_Shield() {
        ...
        if ( false !== strpos( $this->getREQUEST_URI(), "?" ) ) {
            $v = substr( $this->getREQUEST_URI(), strpos( $this->getREQUEST_URI(), "?" ), strlen( $this->
                getREQUEST_URI() ) );
            if ( false !== strpos( $v, "-" ) && ( ( false !== strpos( $v, "?-" ) ) || ( false !== strpos( $v,
                "?+-" ) ) || ( false !== strpos( $this->url_decoder( $v ), "?+-" ) ) ) && ( false === strpos( $this->
                url_decoder( $v ), "=" ) ) ) {
                $r = $this->rprefix . "an attempt to exploit a PHP bug. (CVE-2012-1823)";
                $this->banChecker( $r, true );
                return;
            }
        }
    ...
    function cookieShield() {
        if ( ( false === $this->byPass() ) || ( false !== empty( $_COOKIE ) ) )
            return;

        $_cookie_blacklist = "eval\(|fromCharCode|prompt\(|ZXZhbCg=|ZnJvbUNoYXJDb2Rl|U0VMRUNULyoqLw==|Ki9XSEVSRS8q";
        ...
        if ( false !== ( bool )$injectattempt ) {
            $r = $this->rprefix . "malicious \$_COOKIE content: [ " . stripslashes( $orig_v ) . " ].";
            $this->banChecker( $r, true );
            return;
        }
        $i = 0;
        while ( $i < count( $cnodekeys ) ) {
            ...
            if ( ( is_string( $cnodekey ) ) ) {
                if ( false !== ( bool )preg_match( "/$_cookie_blacklist/i", ( $cnodeval ) ) ) {
                    $r = $this->rprefix . "a banned \$_Cookie string: " . $cnodekey . "=" . $cnodeval;
                    $this->banChecker( $r, true );
                    return;
                }
            }
    ...
    function injectionMatch( $string ) {
        ...
        $string = strtolower( $this->cleanString( 1, $string ) ); // urldecode twice
        ...
        if ( false !== ( bool )preg_match( "/\bdrop\b/i", $string ) && false !== ( bool )preg_match( "/\btable\b|\buser\b/i",
            $string ) && false !== ( bool )preg_match( "/--|\//i", $string ) ) {
            return true;
        } elseif ( ( false !== strpos( $string, "grant" ) ) && ( false !== strpos( $string, "all" ) ) && ( false
        !== strpos( $string, "privileges" ) ) ) {
            return true;
        } elseif ( false !== ( bool )preg_match( "/(?:(sleep\((\s*)(\d*)(\s*)\)|benchmark\((.*)\,(.*)\)))/i",
        $string ) ) {
            return true;
        } elseif ( false !== preg_match_all( "/\bload\b|\bdata\b|\binfile\b|\btable\b|\bterminated\b/i", $string,
        $matches ) > 3 ) {
            return true;
        } elseif ( ( ( false !== ( bool )preg_match( "/select|declare|ascii\(substring|length\(/i", $string ) ) &&
        ( false !== ( bool )preg_match( "/\band\b|\bif\b|\bfrom\b/i", $string ) ) && ( false !== ( bool )
            preg_match( "/$sqlmatchlist/", $string ) ) ) ) {
            return true;
        ...
        } elseif ( ( substr_count( $string, "var" ) > 1 ) && ( false !== ( bool )preg_match( "/date\(|while\(|sleep\(/i",
        $string ) ) ) {
            return true;
        }
        ...
        if ( false !== strpos( $string, "by" ) && ( false !== ( bool )preg_match( "/\border\b|\bgroup\b/i",
            $string ) ) && ( false !== ( bool )preg_match( "/\bcolumn\b|\bdesc\b|\berror\b|\bfrom\b|hav|\blimit\b|\boffset\b|\btable\b|\/|--/i",
            $string ) || ( false !== ( bool )preg_match( "/\b[0-9]\b/i", $string ) ) ) ) {
            return true;
        } elseif ( ( false !== ( bool )preg_match( "/\btable\b|\bcolumn\b/i", $string ) ) && false !== strpos
        ( $string, "exists" ) && ( false !== ( bool )preg_match( "/\bif\b|\berror\b|\buser\b|\bno\b/i", $string ) ) ) {
            return true;
        } elseif ( ( ( false !== strpos( $string, "waitfor" ) && false !== strpos( $string, "delay" ) && ( ( bool )
        preg_match( "/(:)/i", $string ) ) ) || false !== strpos( $string, "nowait" ) ) && ( false !== ( bool )
            preg_match( "/--|\/|\blimit\b|\bshutdown\b|\bupdate\b|\bdesc\b/i", $string ) ) ) {
            return true;
        } elseif ( false !== ( bool )preg_match( "/\binto\b/i", $string ) && ( false !== ( bool )preg_match( "/\boutfile\b/i",
        $string ) ) ) {
            return true;
        } elseif ( false !== ( bool )preg_match( "/\bdrop\b/i", $string ) && ( false !== ( bool )preg_match( "/\buser\b/i",
        $string ) ) ) {
            return true;
        } elseif ( ( ( false !== strpos( $string, "create" ) && false !== ( bool )preg_match( "/\btable\b|\buser\b|\bselect\b/i",
        $string ) ) || ( false !== strpos( $string, "delete" ) && false !== strpos( $string, "from" ) ) || ( false
            !== strpos( $string, "insert" ) && ( false !== ( bool )preg_match( "/\bexec\b|\binto\b|from/i", $string ) ) ) ||
            ( false !== strpos( $string, "select" ) && ( false !== ( bool )preg_match( "/\bby\b|\bcase\b|from|\bif\b|\binto\b|\bord\b|union/i",
            $string ) ) ) ) && ( ( false !== ( bool )preg_match( "/$sqlmatchlist/i", $string ) ) || ( 2 <
            substr_count( $string, "," ) ) ) ) {
            return true;
        } elseif ( ( false !== strpos( $string, "union" ) ) && ( false !== strpos( $string, "select" ) ) && ( false
        !== strpos( $string, "from" ) ) ) {
            return true;
    ...
    function hextoascii( $str ) {
        if ( ( substr_count( $str, "%" ) < 3 ) && ( substr_count( $str, "\x" ) < 1 ) )
            return;
        $p = "";
     ...
     */
    function ipTrapped() {
        if ( ( false !== $this->_useIPTRAP ) && ( ! empty( $this->_ipTrappedURL ) && file_exists( $this->
            _ipTrappedURL ) ) ) {
            ...
            foreach ( $mybans as $i => $value ) {
                if ( strlen( $mybans[$i] > 0 ) ) {
                    ...
                    if ( false !== strpos( $mybans[$i], $this->realIP ) ) {
    ...
    function ipTrapban( $banip ) {
        ...
        if ( ! empty( $this->_ipTrappedURL ) && file_exists( $this->_ipTrappedURL ) ) {
            ...
            foreach ( $mybans as $i => $value ) {
                if ( strlen( $mybans[$i] > 0 ) ) {
                    if ( false !== strpos( $mybans[$i], $newline ) )
                        $bannedAlready = true;
                }
            }
            foreach ( $mybans as $i => $value ) {
                if ( false !== strpos( $mybans[$i], " " ) )
                    $mybans[$i] = preg_replace( "/[\s\r\n]/i", "", $mybans[$i] );
            ...
            if ( false === ( bool )$bannedAlready ) {
                if ( ( false !== strpos( $mybans[$i], $endIPTrapIP ) ) )
                    unset( $mybans[$i] );
                if ( in_array( $limitend, $mybans ) ) {
                    $i = count( $mybans ) - 1;
                    while ( $mybans[$i] != $limitend ) {
    ...
    function checkfilename( $fname ) {
        if ( ( ! empty( $fname ) ) && ( substr_count( $fname, ".php" ) == 1 ) && ( ".php" == substr( $fname,
            -4 ) ) ) {
            if ( ( ( strlen( $fname ) ) - ( strpos( $fname, "." ) ) ) <> 4 ) {
                return false;
            } elseif ( ( false !== file_exists( $fname ) && false !== is_readable( $fname ) ) || ( false !==
            strpos( $_SERVER["SCRIPT_NAME"], "ext/modules/" ) ) )
                return true;
        } else
            return false;
        return false;
    ...
    function byPass() {
        ...
        $filename_bypass = array( "sitemonitor", "protx_process.php", "dps_pxpay_result_handler.php",
            "ipn.php", "express_payflow.php", "quickpay.php", "xml.php", "xml2.php" );
    ...
    function checkReqType() {
        if ( false === $this->byPass() || ( false === $this->_nonGETPOSTReqs ) )
            return;
        ...
        if ( false !== ( bool )preg_match( "/[0-9]+/", $reqType ) ) {
            $r = $this->rprefix . "a request method [ " . $_SERVER["REQUEST_METHOD"] .
                " ] which should not contain numbers. ";
            $this->banChecker( $r, true );
        }
        ...
        if ( false === ctype_upper( $reqType ) ) {
            ...
            $r = $this->rprefix . "a request method [ " . $_SERVER["REQUEST_METHOD"] .
                " ] that is neither GET, OPTIONS, HEAD or POST. ";
            $this->banChecker( $r, true );
        }
    }
    /**
    ...
    function chkSetup() {
        ...
        if ( ( $this->_banipaddress ) && ( $this->_useIPTRAP ) )
            die( "<p align=center><font face=verdana size=1>" . "<strong>WARNING</strong>: Choose either \$banipaddress or \$useIPTRAP, not both thanks.</font></p>" );
        # if using IPTrap, Make sure $ipTrapBlocked is set
        if ( ( $this->_useIPTRAP ) && empty( $this->_ipTrapBlocked ) )
            die( "<p align=center><font face=verdana size=1>" . "<strong>WARNING</strong>: Check the \$ipTrapBlocked url to the IP Trap blocked.php file in the osc.php file for errors.<br />" .
     ...
     */
    function getDir() {
        if ( defined( "DIR_FS_CATALOG" ) && false === strpos( DIR_FS_CATALOG, "http://" ) ) {
            if ( "/" !== substr( DIR_FS_CATALOG, -1 ) )
                return DIR_FS_CATALOG . "/";
            return DIR_FS_CATALOG;
        } elseif ( ! defined( "DIR_FS_CATALOG" ) || ( defined( "DIR_FS_CATALOG" ) && false !== strpos( DIR_FS_CATALOG,
        "http://" ) ) ) {
    ...
    function getRealIP() {
        ...
        if ( isset( $_SERVER ) ) {
            ...
            if ( ! empty( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ) {
                ...
                foreach ( $iplist as $ip ) {
                    $HTTP_X_FORWARDED_FOR = $this->url_decoder( $ip );
                }
                $this->check_ip( $HTTP_X_FORWARDED_FOR );
                if ( isset( $HTTP_X_FORWARDED_FOR ) && ( ! empty( $HTTP_X_FORWARDED_FOR ) ) ) {
        ...
        $serverVars = new ArrayIterator( array( "HTTP_CLIENT_IP", "HTTP_X_FORWARDED", "HTTP_PROXY_USER",
            "HTTP_X_CLUSTER_CLIENT_IP", "HTTP_FORWARDED", "HTTP_CF_CONNECTING_IP", "HTTP_FORWARDED_FOR", ) );
        while ( $serverVars->valid() ) {
            if ( array_key_exists( $serverVars->current(), $_SERVER ) && ! empty( $_SERVER[$serverVars->current
        ...
        return ( false !== $this->check_ip( $_SERVER["REMOTE_ADDR"] ) ) ? $_SERVER["REMOTE_ADDR"] : $this->
            senda403Header();
    }
    function fINT( $integ ) {
        ...
        if ( function_exists( 'filter_var' ) && defined( 'FILTER_SANITIZE_NUMBER_INT' ) ) {
            ...
            if ( isset( $integ ) && $integ_filtered && is_int( $integ_filtered ) && 0 <= $integ_filtered ) {
                return $integ_filtered;
            } else
                return 0;
        } elseif ( isset( $integ ) && is_int( $integ ) && 0 <= ( int )$integ ) {
            return ( int )$integ;
        } else
            return 0;
    }
    ...
    function badArachnid() {
        ...
        if ( false !== $this->issetStrlen( $_SERVER["HTTP_USER_AGENT"] ) ) {
            $badagentlist = array( "Baidu", "WebLeacher", "autoemailspider", "MSProxy", "Yeti", "Twiceler",
                "blackhat", "Mail.Ru", "fuck" );
    ...
    function get_version() {
        if ( false !== file_exists( $this->getDir() . 'includes/version.php' ) && false !== is_readable( $this->
            getDir() . 'includes/version.php' ) ) {
            return trim( implode( '', file( $this->getDir() . 'includes/version.php' ) ) );
        }
        return false;
    }
    function setOpenBaseDir() {
        if ( false !== $this->get_version() && $this->get_version() == "2.3.1" ) {
...
function fix_server_vars() {
    ...
    if ( isset( $_SERVER["SCRIPT_FILENAME"] ) && isset( $_SERVER["PATH_TRANSLATED"] ) && ( strpos( $_SERVER["SCRIPT_FILENAME"],
