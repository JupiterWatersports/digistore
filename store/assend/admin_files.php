<?php
/*
  $Id: admin_files.php,v 1.29 2005/11/01 17:52:23 harley_vb Exp $   
   ============================================  
  ...
  if ($HTTP_GET_VARS['action']) {
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'box_remove':
        ...
        break;
      case 'file_store':
		if(isset($_POST['admin_groups_id'])){	
			$admin_group_ids = '';
			foreach($_POST['admin_groups_id'] as $id){
				$str .= $id.",";
			}
			
			$admin_group_ids = rtrim($str,"','");
		}	
        ...
        $sql_data_array = array('admin_files_name' => tep_db_prepare_input($_POST['admin_files_name']),
							   ...
							   'admin_groups_id' => $admin_group_ids);
        tep_db_perform(TABLE_ADMIN_FILES, $sql_data_array);
...
?>
                    ...
                    <td class="dataTableContent" align="right">
                      <?php if ( (is_object($fInfo)) && ($files['admin_files_id'] == $fInfo->admin_files_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $HTTP_GET_VARS['cPath'] . '&fID=' . $files['admin_files_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
                      &nbsp;</td>
                    </tr>
<?php
  }
...
?>
                    ...
                    <td class="dataTableContent" align="right">
                      <?php if ( (is_object($cInfo)) && ($boxes[$i]['admin_boxes_id'] == $cInfo->admin_boxes_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cID=' . $db_cat['admin_boxes_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
                      &nbsp;</td>
                    </tr>
<?php
   $i++;
                      ...
                      <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                          <tr> 
                            <td class="smallText" valign="top">
                              <?php  echo TEXT_COUNT_BOXES . $boxnum; ?>
                            </td>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'store_file':
      ...
      $contents[] = array('align' => 'left', 'text' => '<br>&nbsp;' . tep_draw_pull_down_menu('admin_files_name', $show, $show, 'class="form-control"').
	'</br>');
		  
		  
		  $get_admin_users_query = tep_db_query("SELECT * FROM admin_groups ORDER BY admin_groups_name ASC");
		  while($get_admin_users = tep_db_fetch_array($get_admin_users_query)){
			  $contents[] = array('text' => tep_draw_checkbox_field('admin_groups_id[]', $get_admin_users['admin_groups_id'], 'checked').$get_admin_users['admin_groups_name']);
		  }
