...
function rewriteDiv(field, stack) {
  ...
  try{document.getElementById("historyMessageStack").innerHTML ='<?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?>' ;}catch(e){}
  ...
  if (stack == 'history') {
  try{document.getElementById("historyMessageStack").innerHTML ='<table><tr class="messageStackSuccess"><td class="messageStackSuccess"><?php echo tep_image(DIR_WS_ICONS . 'success.gif', ICON_SUCCESS) . '&nbsp;' . sprintf(AJAX_MESSAGE_STACK_SUCCESS, 'field'); ?></td></tr></table>' ;}catch(e){}
...
function deleteRow(info, div) {
	info.parentNode.parentNode.parentNode.style.display='none';
...
function updateOrdersField2(field, value) {
		...
		$.post( "<?php echo FILENAME_ORDERS_EDIT_AJAX; ?>", { 
		action: 'update_order_field2' , oID: '<?php echo $_GET['oID']; ?>' , field: field , new_value: value })
		.done(function( data ) {
			rewriteDiv(data, 'orders');
		});
...
function updateProductsField(action, pid, field, value, info) {
  ...
  if ( (action == 'update') || (action == 'reload1') ) {
    if(admin_override == false || (field != 'products_model' && field != 'products_name') ){
      ...
      xmlHttp.send(null);
    }else if(info.substring(0, 9) == "Ebay Item"){
      if(decodeURIComponent(value).substring(0, 9) != "Ebay Item"){
        value = "Ebay Item " + decodeURIComponent(value);
        console.log(value);
      ...
      xmlHttp.send(null);
      location.reload();
    }else if(info.substring(0, 21) == "Repair Inspection Fee"){
      console.log(info.substring(0, 21));
      if(decodeURIComponent(value).substring(0, 21) != "Repair Inspection Fee"){
        value = "Repair Inspection Fee " + decodeURIComponent(value);
        value = encodeURIComponent(value);
      }
  ...
  if (action == 'reload2') {
    ...
    var url = "<?php echo FILENAME_ORDERS_EDIT_AJAX; ?>?action=update_product_value_field&oID=<?php echo $_GET['oID']; ?>&pid=" + pid + "&final_price=" + final_price;
 ...
 function updateProductsTax(){
	 var data = "";
	 data = "&action=sumthinsumthin&";
	 data += "oID=<?php echo $_GET['oID']; ?>";
    document.getElementById("productsTable").innerHTML = '<div align="center"><img src="order_editor/images/working.gif"><br><?php echo AJAX_WORKING; ?><br></div>'; 
...
function updateCommentsField(action, id, oID, value, info) {
	if (action == 'delete'){
...
function updateShippingZone(field, value) {
  ...
  function clearComments() {
   ...
   function getNewComment() { //this is used for inserting new commments
      var data = {
		  action : 'insert_new_comment',
		  oID : <?php echo $_GET['oID']; ?>,
		  status : $("#status").val(),
		  notify : document.getElementById("notify").checked,
		  notifyComments : document.getElementById("notify_comments").checked,
...
function obtainTotals(){
	 var data = "";
	 data = $("#edit_order").serialize();
	 data += "&action=reload_totals&";
	 data += "oID=<?php echo $_GET['oID']; ?>";
   ...
   document.getElementById("payment-date-block").innerHTML = '<div align="center"><img src="order_editor/images/working.gif"><br><?php echo AJAX_WORKING; ?><br></div>'; 
   $.ajax({
  type : 'POST',
  url  : 'edit_orders_ajax.php',
  data : data,
...
function init()
{
  ...
  //calculates all the different values as new entries are typed
	...
	var finalPriceValue = document.getElementById("update_products[" + pid + "][final_price]").value;
