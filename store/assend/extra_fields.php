<?php
/*
  $Id: extra_fields.php ver 3.0 by Kevin L. Shelton 2011-02-09
...
*/
  ...
  $product_types = epf_build_ptype_pulldown();
  ...
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      case 'insert': // validate form
        $all_types = ($HTTP_POST_VARS['all_types'] == '0') ? 0 : 1;
        $ptypes = array();
        foreach ($product_types as $type) {
          if (isset($HTTP_POST_VARS['usetype' . $type['id']]) && ($HTTP_POST_VARS['usetype' . $type['id']] == $type['id'])) $ptypes[] = $type['id'];
        }
        if (empty($ptypes) || in_array(0, $ptypes)) $all_types = 1; // if no types selected set all types to true
        if ($all_types)  $ptypes = array(); // force applicable types to empty if all types selected
        ...
        if ($error) {  // if error return to entry form
          ...
          $data_array = array('epf_order' => (int)$order,
                              ...
                              'epf_all_ptypes' => $all_types,
                              'epf_ptype_ids' => (!empty($ptypes) ? implode('|', $ptypes) : 'null'));
      ...
      case 'update': // validate form
        ...
        if (empty($ptypes) || in_array(0, $ptypes)) $all_types = 1; // if no types selected or set all types to true
        if ($all_types)  $ptypes = array(); // force applicable types to empty if all types selected
      ...
      case 'link_type':
        if ($confirm == 'yes') {
          ...
          $source = tep_db_fetch_array($query);
          if ($source['epf_multi_select'] != 1) {
            $error = true;
            $messageStack->add_session(ERROR_NOT_TYPE_LINKABLE, 'error');
          }
          if ($source['epf_has_linked_field'] != 0) {
            $error = true;
            $messageStack->add_session(ERROR_ALREADY_LINKED, 'error');
          }
          if (!$error) {
            tep_db_query('update ' . TABLE_EPF . ' set epf_has_linked_field = 2, epf_links_to = 0 where epf_id = ' . (int)$eid);
            $messageStack->add_session(sprintf(TEXT_LINK_SUCCESS, $eid, $link_to), 'success');
          }
          tep_redirect(tep_href_link(FILENAME_EXTRA_FIELDS, 'eid=' . $eid));
        }
        ...
        if ($confirm == 'yes') {
          if (isset($HTTP_GET_VARS['used']) && ($HTTP_GET_VARS['used'] > 0)) {
            ...
            if ($field_info['epf_has_linked_field'] != 2) {
              $messageStack->add_session(ERROR_NOT_TYPE_LINKED, 'error');
              tep_redirect(tep_href_link(FILENAME_EXTRA_FIELDS, 'eid=' . $eid));
            }
            tep_db_query('update ' . TABLE_EPF_VALUES . ' set value_depends_on = 0 where epf_id =' . (int)$eid);
            tep_db_query('update ' . TABLE_EPF . ' set epf_has_linked_field = 0, epf_links_to = 0 where epf_id = ' . (int)$eid);
            tep_redirect(tep_href_link(FILENAME_EXTRA_FIELDS, 'eid=' . $eid));
          }
        } else {
...
<!-- header_eof //-->
 <table border="0" width="100%" cellspacing="0" cellpadding="2">
      ...
      if ($action == 'edit') {
        ...
        $applicable_types = explode('|', $field['epf_ptype_ids']);
      ...
      echo "</table>\n";
      echo '<p>' . ENTRY_ALL_TYPES . tep_draw_radio_field('all_types', '1', true, $field['epf_all_ptypes']) . '&nbsp;' . TEXT_YES . '&nbsp;' . tep_draw_radio_field('all_types', '0', false, $field['epf_all_ptypes']) . '&nbsp;' . TEXT_NO . "</p>\n";
      ...
      echo '<hr><p>' . ENTRY_CHECK_TYPES . "<br />\n";
      foreach ($product_types as $type) {
        echo tep_draw_checkbox_field('usetype' . $type['id'], $type['id'], in_array($type['id'], $applicable_types)) . $type['text'] . "<br />\n";
      }
      echo "</p>\n";
      ...
      <?php } elseif ($action == 'delete') { // Confirm deletion of field
        ...
        echo "</td></tr>\n";
      } elseif ($action == 'link_type') { // get field to link to
        $error = false;
        $messages = array();
        echo '<tr><td><p class="pageHeading">' . BUTTON_LINK_PTYPE . "</p>\n";
        ...
        if ($field_info['epf_multi_select'] != 1) {
          $error = true;
          $messages[] = ERROR_NOT_TYPE_LINKABLE;
        }
        if ($field_info['epf_has_linked_field'] != 0) {
          $error = true;
          $messages[] = ERROR_ALREADY_LINKED;
        }
        if ($error) {
          if (!empty($messages)) {
          ...
          echo '<p>' . tep_draw_form('link_type', FILENAME_EXTRA_FIELDS, 'confirm=yes&action=link_type');
          echo tep_draw_hidden_field('epf_id', $eid) . '<b>' . TEXT_CONFIRM_TYPE_LINK . "</b></p>\n";
          echo '<p>' . tep_draw_input_field('', BUTTON_CREATE_TYPE_LINK, 'alt="' . BUTTON_CREATE_TYPE_LINK . '"', false, 'submit') . '&nbsp;&nbsp;';
        }
...
<?php
        ...
        echo "</td></tr>\n";
      } elseif ($action == 'unlink_type') { // Confirm unlink from product type
        $query = tep_db_query("select * from " . TABLE_EPF . " where epf_id = " . (int)$eid);
        $field_info = tep_db_fetch_array($query);
        echo '<tr><td><p class="pageHeading">' . sprintf(HEADING_UNLINK_TYPE, $eid) . "</p>\n";
        $used = 0;
        if ($field_info['epf_has_linked_field'] != 2) {
          $messageStack->add_session(ERROR_NOT_TYPE_LINKED, 'error');
          tep_redirect(tep_href_link(FILENAME_EXTRA_FIELDS, 'eid=' . $eid));
        ...
        $check = tep_db_fetch_array($check_query);
        $used = $check['total']; // how many values are linked
        echo '<p>' . sprintf(TEXT_NUM_TYPE_LINKED, $used) . "</p>\n";
        if ($double_check == 'no') {
          echo '<p>' . TEXT_SURE_UNLINK . (($used > 0) ? TEXT_LINKS_GONE : '') . "</p>\n";
          echo '<p>' . tep_draw_form('yes', FILENAME_EXTRA_FIELDS, 'confirm=yes&action=unlink_type&eid=' . $eid . '&used=' . $used) . tep_draw_input_field('yes', TEXT_YES, 'alt="' . TEXT_YES . '"', false, 'submit') . '</form>&nbsp;&nbsp;';
          echo tep_draw_form('no', FILENAME_EXTRA_FIELDS, 'eid=' . $eid) . tep_draw_input_field('no', TEXT_NO, 'alt="' . TEXT_NO . '"', false, 'submit') . "</form></p>\n";
        } else {
          echo '<p><b>' . TEXT_CONFIRM_UNLINK . "</b></p>\n";
          echo '<p>' . tep_draw_form('yes', FILENAME_EXTRA_FIELDS, 'confirm=yes&action=unlink_type&eid=' . $eid) . tep_draw_input_field('yes', TEXT_YES, 'alt="' . TEXT_YES . '"', false, 'submit') . '</form>&nbsp;&nbsp;';
          echo tep_draw_form('no', FILENAME_EXTRA_FIELDS, 'eid=' . $eid) . tep_draw_input_field('no', TEXT_NO, 'alt="' . TEXT_NO . '"', false, 'submit') . "</form></p>\n";
        }
        echo "</td></tr>\n";
...
<?php // build information box contents
  ...
  if (isset($selected)) {
    ...
    if ($selected['epf_has_linked_field']) {
      $contents[] = array('align' => 'center', 'text' => tep_draw_form('unlink', FILENAME_EXTRA_FIELDS, 'eid=' . $eid . '&action=' . (($selected['epf_has_linked_field'] == 1) ? 'unlink' : 'unlink_type')) . tep_draw_input_field('unlink', BUTTON_UNLINK, 'alt="' . BUTTON_UNLINK . '"', false, 'submit') . '</form>');
      ...
      if ($selected['epf_multi_select']) {
        $contents[] = array('align' => 'center', 'text' => tep_draw_form('plink', FILENAME_EXTRA_FIELDS, 'eid=' . $eid . '&action=link_type') . tep_draw_input_field('ptlink', BUTTON_LINK_PTYPE, 'alt="' . BUTTON_LINK_PTYPE . '"', false, 'submit') . '</form>');   
      }
    }
    ...
    $contents[] = array('text' => TEXT_APPLIES_TYPES . ($selected['epf_all_ptypes'] ? TEXT_ALL : TEXT_LIMITED));
    ...
    if ($selected['epf_has_linked_field'] == 1) {
      $contents[] = array('text' => TEXT_LINKED_TO . $selected['epf_links_to']);
    } elseif ($selected['epf_has_linked_field'] == 2) {
      $contents[] = array('text' => TEXT_LINKED_TO . TEXT_PTYPE);
    }
  }
