...
/*abstract*/ class attributeManager /*implements attributeManagerInterface*/ {
	...
	function getAllOptionValues() {
		if(0 === count($this->arrAllOptionValues)) {
			$query = amDB::query("select * from ".TABLE_PRODUCTS_OPTIONS_VALUES." where language_id='".amDB::input($this->intLanguageId)."' ORDER BY products_options_values_name ");
			while($res = amDB::fetchArray($query)){
	...
	function getAllOptionsAndValues() {
		if(0 === count($this->arrAllOptionsAndValues)){
			...
			foreach($this->arrAllOptionsAndValues as $oID=>$oV){
				$vals = $oV['values'];
				$newV = array();
				if (is_array($vals)) {
				foreach($vals as $k=>$v){
					$v2=explode(' ',str_replace(array('/','-'),' ',$v));
					$newV[]=array($k,$v,strtoupper($v2[0]));
				}
				}
				$c = count($newV);
				for($i=0;$i<$c;$i++)
					for($j=$i+1;$j<$c;$j++){
						...
						$n2 = (float) $newV[$j][1];
						if($n1==0){
							if($newV[$i][2]=='XS') $n1=9999991;
							elseif($newV[$i][2]=='S') $n1=9999992;
							elseif($newV[$i][2]=='M') $n1=9999993;
							elseif($newV[$i][2]=='L') $n1=9999994;
							elseif($newV[$i][2]=='XL') $n1=9999995;
							elseif($newV[$i][2]=='XXL') $n1=9999996;
							elseif($newV[$i][2]=='XXXL') $n1=9999997;
							elseif($newV[$i][1][0]!=='0') $n1=99999999;
						}
						if($n2==0){
							if($newV[$j][2]=='XS') $n2=9999991;
							elseif($newV[$j][2]=='S') $n2=9999992;
							elseif($newV[$j][2]=='M') $n2=9999993;
							elseif($newV[$j][2]=='L') $n2=9999994;
							elseif($newV[$j][2]=='XL') $n2=9999995;
							elseif($newV[$j][2]=='XXL') $n2=9999996;
							elseif($newV[$j][2]=='XXXL') $n2=9999997;
							elseif($newV[$j][1][0]!=='0') $n2=99999999;
						}
						//if($n1==0 && $newV[$i][1][0]!=='0')$n1=99999999;
						//if($n2==0 && $newV[$j][1][0]!=='0')$n2=99999999;
						if($n1>$n2 || ($n1==$n2 && $newV[$i][1]>$newV[$j][1])){
							$t=$newV[$i];
							$newV[$i]=$newV[$j];
							$newV[$j]=$t;
						}
				...
				$vals = array();
				foreach($newV as $r) $vals[$r[0]]=$r[1];
				$this->arrAllOptionsAndValues[$oID]['values']=$vals;
			}
		}
