...
function addAttributeToProduct($get) {
  ...
  if(isset($get['quantity_id']) === true) {
    $this->getAndPrepare('quantity_id', $get, $getArray['quantity_id']);
  }

  if(isset($get['model_no']) === true) {
    $this->getAndPrepare('model_no', $get, $getArray['model_no']);
  }
  if(isset($get['serial_no']) === true) {
    $this->getAndPrepare('serial_no', $get, $getArray['serial_no']);
  }
  if(isset($get['quantity']) === true) {
    $this->getAndPrepare('quantity', $get, $getArray['quantity']);
  }
...
// QT pro
	...
	function update($get) {
		...
		foreach($this->arrSessionVar as $id => $res) {
			if(($res['option_id'] == $getArray['option_id']) && ($res['option_value_id'] == $getArray['option_value_id'])) {
				...
				$this->arrSessionVar[$id]['quantity_id'] = $getArray['quantity_id'];
				$this->arrSessionVar[$id]['model_no'] = $getArray['model_no'];
				$this->arrSessionVar[$id]['serial_no'] = $getArray['serial_no'];
				$this->arrSessionVar[$id]['quantity'] = $getArray['quantity'];
	...
	function update2($get) {
          ...
          if(isset($get['option_value_id']) === true) {
		   ...
		   if(isset($get['products_attributes_id']) === true) {
            $this->getAndPrepare('products_attributes_id', $get, $getArray['products_attributes_id']);
          }
	...
	function getAllProductOptionsAndValues($reset = false) {
		...
		for($z1=0;$z1<count($this->arrSessionVar);$z1++){
			...
			foreach($this->arrSessionVar as $id => $res) {
				...
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['quantity_id'] = $res['quantity_id'];
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['model_no'] = $res['model_no'];
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['serial_no'] = $res['serial_no'];
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['quantity'] = $res['quantity'];
