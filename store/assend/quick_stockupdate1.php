  ...
  if(isset($_POST['action']) && $_POST['action']=='ajaxupdate'){
	...
	if($options &&  $pid){
		foreach($options as $opt){
			if($opt['value'] != ''){
				$sql = "UPDATE products_attributes SET options_quantity = '".$opt['value']."' WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']."";
				tep_db_query($sql );
			}
		}
	}
...
echo '<td>&nbsp;<!--<input type="hidden" name="stock_update[opt]['.$products_options['products_options_values_id'].'][options_values_id]" value="'. $products_options['products_options_values_id'].'">--></td><td>&nbsp;</td>';
 ...
 ?>
           ...
           <input name="stock_update[opt][<?php echo $products_options['products_options_values_id'];?>][<?php echo $products_options['options_id'].'-'.$results['products_id'];?>][value_quantity]" type="text" class="prodfield_<?php echo  $results['products_id'] ?>"   value="<?php echo $products_options['options_quantity']; ?>" size="6"></td></tr>
<?php
          ...
          <td valign="top">
           ...
           <table border="0" width="100%" cellspacing="2" cellpadding="2">
            ...
            <tr class="dataTableHeadingRow" style="background-image:none;">
             ...
             <td class="dataTableHeadingContent" width="3%" align="center"><?php echo QUICK_WEIGHT; ?></td>
