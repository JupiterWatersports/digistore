<?php
/*
  $Id: extra_values.php ver 3.0 by Kevin L. Shelton 2011-02-25
...
// set the following two defines to the maximum size you want an extra value image to have
  define('EPF_IMAGE_MAX_WIDTH', 50);
  define('EPF_IMAGE_MAX_HEIGHT', 50);
  ...
  function resizeWithGD($image, $max_width = EPF_IMAGE_MAX_WIDTH, $max_height = EPF_IMAGE_MAX_HEIGHT) {
    ...
    if ($img_type !== false) {
      list($orig_width, $orig_height) = getimagesize(DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
      ...
      switch ($img_type) {
        case 'jpg':
          $im = imagecreatefromjpeg(DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
        case 'gif':
          $im = imagecreatefromgif(DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
        case 'png':
          $im = imagecreatefrompng(DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
      }
      $true_color = true;
      ...
      switch ($img_type) {
        case 'jpg':
          imagejpeg($im_p, DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
        case 'gif':
          imagegif($im_p, DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
        case 'png':
          imagepng($im_p, DIR_FS_CATALOG_IMAGES . 'temporary/' . $image);
          break;
      }
      imagedestroy($im_p);
      imagedestroy($im);
      chmod(DIR_FS_CATALOG_IMAGES . 'temporary/' . $image, 0777);
    } else {
      return false;
    }
  }
  ...
  $list_id = (isset($HTTP_GET_VARS['list_id']) ? $HTTP_GET_VARS['list_id'] : '_');
  ...
  if ($current_label == '') { // if invalid label get first matching label and use that information
    ...
    $list_id = $eid . '_' . $lid;
    $HTTP_GET_VARS['list_id'] = $list_id; // set so dropdown list of fields will match value list chosen
  }
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      case 'insert':
        $value = (isset($HTTP_POST_VARS['value']) ? tep_db_prepare_input($HTTP_POST_VARS['value']) : '');
        $order = (isset($HTTP_POST_VARS['sort_order']) ? tep_db_prepare_input($HTTP_POST_VARS['sort_order']) : 0);
        $depends_on = (isset($HTTP_POST_VARS['depends_on']) ? tep_db_prepare_input($HTTP_POST_VARS['depends_on']) : 0);
        $excludes = (isset($HTTP_POST_VARS['excludes']) ? $HTTP_POST_VARS['excludes'] : array());
        ...
        $value_image->set_destination(DIR_FS_CATALOG_IMAGES . 'temporary/');
        if ($value_image->parse() && $value_image->save()) {
            $source = DIR_FS_CATALOG_IMAGES . 'temporary/' . $value_image->filename;
            resizeWithGD($value_image->filename);
        }
        if (!$error) {
          tep_db_perform(TABLE_EPF_VALUES, $data_array);
          $vid = tep_db_insert_id();
          if (tep_not_null($source)) {
            $ext = '';
            if ($image = getimagesize($source)) {
              switch ($image[2]) {
                case IMAGETYPE_JPEG:
                  $ext = 'jpg';
                  break;
            ...
            if ($ext != '') {
              $new_file_name = 'epfv' . $vid . '.' . $ext;
              $dest = DIR_FS_CATALOG_IMAGES . 'epf/' . $new_file_name;
              $success = copy($source, $dest); //move
              if ($success) {
                @chmod($dest, 0777);
                $data_array['value_image'] = $new_file_name;
      ...
      case 'update': // validate form
        ...
        if (tep_not_null($source)) {
          ...
          @unlink($source); // remove temporary file
          $check_query = tep_db_query('select value_image from ' . TABLE_EPF_VALUES . ' where value_id = ' . (int)$vid);
          $check = tep_db_fetch_array($check_query);
          if (($check['value_image'] != '') && ($new_file_name != $check['value_image'])) {
            // image file name has changed, remove old file
            if (file_exists(DIR_FS_CATALOG_IMAGES . 'epf/' . $check['value_image'])) {
              @unlink(DIR_FS_CATALOG_IMAGES . 'epf/' . $check['value_image']);
            }
          }
        }
        if (!$error) {
 ...
 <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td colspan=2 align="right" class="main"><?php echo tep_draw_form('new', FILENAME_EXTRA_VALUES, 'action=new&list_id=' . $list_id) . tep_draw_input_field('new', BUTTON_NEW, 'alt="' . BUTTON_NEW . '"', false, 'submit') . '</form> &nbsp;&nbsp; ';
            if (!$field_info['epf_multi_select']) echo tep_draw_form('sub', FILENAME_EXTRA_VALUES, "", 'get') . tep_draw_hidden_field('action', 'new') . tep_draw_hidden_field('list_id', $list_id) . BUTTON_SUBVALUE . ': ' . tep_draw_pull_down_menu('parent', tep_build_epf_pulldown($eid, $lid, array(array('id' => 0, 'text' => TEXT_NONE))), 0, 'onChange="this.form.submit();"') . '</form>&nbsp;&nbsp;'; ?></td>
          </tr>
          <?php } ?>
        </table></td>
      </tr>
      ...
      if ($field_info['epf_multi_select']) {
        if ($field_info['epf_has_linked_field']) {
          if ($field_info['epf_has_linked_field'] == 2) { // linked to product type
            $full_list = epf_build_ptype_pulldown(0, array(array('id' => 0, 'text' => ENTRY_NO_VALUE_REQUIRED)));
            if ($field_info['epf_all_ptypes']) {
              $link_values = $full_list;
            } else { // field is only valid for certain types, values can only require one of those types
              $ptypes =array();
              $base_types = explode('|', $field_info['epf_ptype_ids']);
              $all_epf_types = array();
              ...
              $link_values = array(array('id' => 0, 'text' => ENTRY_NO_VALUE_REQUIRED));
              foreach ($full_list as $item) {
                if (in_array($item['id'], $ptypes)) $link_values[] = array('id' => $item['id'], 'text' => $item['text']);
              }
            }
          } else { // linked to list field
...
?>
              ...
              if ($field_info['epf_has_linked_field'] && $field_info['epf_multi_select']) {
                ...
                $value = tep_db_fetch_array($query);
                if ($field_info['epf_has_linked_field'] == 2) {
                  $label = TEXT_PTYPE;
                } else {
                ...
                foreach ($vallist as $required => $values) {
                  if ($required == 0) {
                    ...
                    if ($field_info['epf_has_linked_field'] == 2) {
                      $check = epf_get_ptype_children($required);
                    } else {
                    ...
                    foreach ($check as $val) {
                      if ($field_info['epf_has_linked_field'] == 2) {
                        $reqlist[] = epf_get_ptype_desc($val);
                      } else {
  ...
  if (!empty($selected)) {
    ...
    if (!$field_info['epf_multi_select']) {
      ...
      if ($selected['parent_id'] > 0)
        $contents[] = array('align' => 'center', 'text' => tep_draw_form('goparent', FILENAME_EXTRA_VALUES, 'vid=' . $selected['parent_id'] ) . tep_draw_input_field('gotoparent', BUTTON_SELECT_PARENT, 'alt="' . BUTTON_SELECT_PARENT . '"', false, 'submit') . '</form>');
    }
    $query = tep_db_query("select value_id, epf_value from " . TABLE_EPF_VALUES . " where parent_id = " . (int)$selected['value_id']);
    $clist = array(array('id' => 0, 'text' => TEXT_NONE));
    while ($val = tep_db_fetch_array($query)) {
      $clist[] = array('id' => $val['value_id'], 'text' => $val['epf_value']);
    }
    if (sizeof($clist) > 1) {
      $contents[] = array('text' => TEXT_CHILD . tep_draw_form('tosubval', FILENAME_EXTRA_VALUES, "", 'get') . tep_draw_pull_down_menu('vid', $clist, 0, 'onChange="this.form.submit();" style="font-size: 10px; width: 100px"') . '</form>&nbsp;&nbsp;');
    }
    ...
    if ($selected['value_depends_on'] != 0) {
      if ($field_info['epf_has_linked_field'] == 2) {
        $required = epf_get_ptype_desc_extended($selected['value_depends_on']);
      } else {
        $required = tep_get_extra_field_list_value($selected['value_depends_on']);
      }
      $contents[] = array('text' => TEXT_REQUIRES . '<br>' . $required);
    }
  }
