...
*/
 ...
 // if ($_POST['action'] != 'process') {
  ...
  //}
$admin_query =  tep_db_query("select admin_firstname, admin_lastname from admin where admin_id = '".$login_id."' ");
$admin = tep_db_fetch_array ($admin_query); 
  
  
  if ($_POST['customers_create_type'] == 'existing') {
  ...
  $delivery_location = tep_db_prepare_input($_POST['delivery_location']);
  $new_value = "1";
  ...
  $customer_service_id = tep_db_prepare_input($admin['admin_firstname'].' '.$admin['admin_lastname']);
  ...
  if ($_POST['customers_create_type'] == 'new') {
	...
	if (!empty($customers_password)) {
			...
			$sql_data_array['guest_account'] = '0';
		}else{
			$sql_data_array['guest_account'] = '1';
		}
...
/*
if ((MAILCHIMP_ENABLE == 'true' && $customers_newsletter == '1'
	)) {
    ...
    tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
      ...
      if (defined('MODULE_ORDER_TOTAL_INSTALLED') && tep_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
		...
		$sql_data_array = array('orders_id' => $insert_id,
			...
			tep_db_query('INSERT INTO orders_total (`orders_id`, `title`, `text`, `value`, `class`, `sort_order`) VALUES ('.$insert_id.', "Sub-Total:" , "0.00", "0", "ot_subtotal", "1"), 
	('.$insert_id.', "Tax" , "0.00", "0", "ot_tax", "3"), 
	('.$insert_id.', "Total" , "0.00", "0", "ot_total", "9")');
      }
...
*/
   tep_redirect(tep_href_link(FILENAME_ORDERS_EDIT, 'oID=' . $insert_id.'&act=create_order', 'SSL'));
