<?php
/*
  $Id: admin_members.php,v 1.29 2005/11/01 17:52:23 harley_vb Exp $   
   ============================================  
    ...
    switch ($HTTP_GET_VARS['action']) {
      case 'member_new':
        ...
        if (in_array($HTTP_POST_VARS['admin_email_address'], $stored_email)) {
          ...
          $sql_data_array = array('admin_groups_id' => tep_db_prepare_input($HTTP_POST_VARS['admin_groups_id']),
                                  ...
                                  'admin_password' => tep_encrypt_password('othu7791'),
                                  'admin_created' => 'now()');
      ...
      case 'member_edit':
        ...
        if (in_array($HTTP_POST_VARS['admin_email_address'], $stored_email)) {
          ...
          //if($HTTP_POST_VARS['admin_firstname'] == 'Vikas'){
            //$sql_data_array = array('admin_groups_id' => tep_db_prepare_input($HTTP_POST_VARS['admin_groups_id']),
              //'admin_firstname' => tep_db_prepare_input($HTTP_POST_VARS['admin_firstname']),
              //'admin_lastname' => tep_db_prepare_input($HTTP_POST_VARS['admin_lastname']),
              //'admin_email_address' => tep_db_prepare_input($HTTP_POST_VARS['admin_email_address']),
              //'admin_modified' => 'now()',
              //'admin_password' => tep_encrypt_password('vthu9779'));
          //}else{
            $sql_data_array = array('admin_groups_id' => tep_db_prepare_input($HTTP_POST_VARS['admin_groups_id']),
              'admin_firstname' => tep_db_prepare_input($HTTP_POST_VARS['admin_firstname']),
              'admin_lastname' => tep_db_prepare_input($HTTP_POST_VARS['admin_lastname']),
...
?>
                    <tr class="dataTableRowBoxes"> 
                      <td colspan=2 class="dataTableContent" valign="top" align="right">
                        <?php if ($HTTP_GET_VARS['gPath'] != 1) { echo  '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS, 'gID=' . $HTTP_GET_VARS['gPath']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a> ' . tep_image_submit('button_save.gif', IMAGE_INSERT); } else { echo tep_image_submit('button_back.gif', IMAGE_BACK); } ?>
                        &nbsp;</td>
                    </tr>
                  </table></form>
<?php
...
?>
                    ...
                    <td class="dataTableContent" align="right">
                      <?php if ( (is_object($gInfo)) && ($groups['admin_groups_id'] == $gInfo->admin_groups_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS, 'gID=' . $groups['admin_groups_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
                      &nbsp;</td>
                    </tr>
<?php
    $count_groups++;
...
<?php
  $db_admin_query_raw = "select * from " . TABLE_ADMIN . " where deleted_at is null order by admin_firstname";
...
?>
                    ...
                    <td class="dataTableContent" align="right">
                      <?php if ( (is_object($mInfo)) && ($admin['admin_id'] == $mInfo->admin_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ADMIN_MEMBERS, 'page=' . $HTTP_GET_VARS['page'] . '&mID=' . $admin['admin_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
                      &nbsp;</td>
                    </tr>
<?php
  }
?>
                    <tr> 
                      <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                          <tr> 
                            <td class="smallText" valign="top"><?php echo $db_admin_split->display_count($db_admin_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_MEMBERS); ?><br>
