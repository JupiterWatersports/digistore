...
foreach ($custid as $cid){
	unset($email);
	  ...
	  $knt = tep_db_num_rows($query4);
	  
	$email .= '<table style="width:600px; display:table; margin:0px auto;"><tr><td style="width:100%; text-align:center;"><img style="width:100%; max-width:250px; text-align:center;" src="http://www.jupiterkiteboarding.com/store/images/jup-kitepaddlewake.png" alt="Jupiter"></td></tr>
	<tr><td><hr style="margin-bottom:20px;"></td></tr>
	<tr><td><h1>Don&#39t Forget!</h1></td></tr>
	<tr><td>We noticed that during a visit to our store you placed the following item(s) in your shopping cart, but did not complete the transaction.</td></tr>';
	
	$products_line = '';
	$mline = '';
	  while($inrec3 = tep_db_fetch_array($query4)){
		...
		// set new cline and curcus
		 if ($lastcid != $cid) {
			...
			$cline = "<tr> <td class='dataTableContent' align='left' colspan='6' nowrap><a href='" . tep_href_link(FILENAME_CUSTOMERS, 'search=' . $inrec3['lname'], 'NONSSL') . "'>" . $inrec3['fname'] . " " . $inrec3['lname'] . "</a>".$customer."</td></tr>";
			$tprice = 0;
		  }
		  $lastcid = $cid;
		...
		$query2 = tep_db_query("select  p.products_price price,
                            ...
                            from    " . TABLE_PRODUCTS . " p,
                                    ...
                                    " . TABLE_LANGUAGES . " l
                            where   p.products_id = '" . $inrec3['pid'] . "' and
                                    pd.products_id = p.products_id and
                                    pd.language_id = " . (int)$languages_id );

		$inrec2 = tep_db_fetch_array($query2);
		$sprice = tep_get_products_special_price( $inrec3['pid'] );
		if( $sprice < 1 )
			$sprice = $inrec2['price'];
      ...
      $tpprice_formated = $currencies->format(($inrec3['qty'] * $sprice));
	  
      $cline .= "<tr class='dataTableRow'>
                    <td class='dataTableContent' align='left'   width='15%' nowrap>" . $inrec2['model'] . "</td>
                    <td class='dataTableContent' align='left'  colspan='2' width='55%'><a href='" . tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $inrec3['pid'] . '&origin=' . FILENAME_RECOVER_CART_SALES . '?page=' . $HTTP_GET_VARS['page'], 'NONSSL') . "'>" . $inrec2['name'] . "</a></td>
                    <td class='dataTableContent' align='center' width='10%' nowrap>" . $inrec3['qty'] . "</td>
...
$products_line .= '<tr style="float:left;width:100%; padding-bottom: 20px; border-bottom: 1px solid #ddd; margin-bottom:20px;">
					<td style="padding-right:20px;"><a href="http://www.jupiterkiteboarding.com/store/product_info.php?products_id='.$inrec2['products_id'].'" target="_blank">'.tep_image('http://www.jupiterkiteboarding.com/store/'.DIR_WS_IMAGES .''.$inrec2['products_image'], $inrec2['products_name'], '100' , '100') . '</a></td>
                	
					<td style="padding-top:20px; vertical-align:top;">
					<a style="text-decoration:none; color:#000;" targe="_blank" href="http://www.jupiterkiteboarding.com/store/product_info.php?products_id='. $inrec2['products_id'] . '"><b>' . $inrec2['name'] . '</b></a>
					</br><span style="font-size:14px;">'.$pprice_formated.'</span></td></tr>
					
					';
	  }
	  
	  $query5 = tep_db_query("select cb.products_id pid
	...
	$output = '';
	while($product2 = tep_db_fetch_array($query5)){
		$output .= ''.$product2['pid'].',' ;
	} 		
		if($_POST['discount_code'] == '1'){
 	  $mline .= '<tr><td style="text-align:center;">Use the following Discount Code at checkout to recieve 5% off your entire order: <b>KITE5</b></td></tr>
	  			 <tr><td>&nbsp;</td></tr>';
		}
	  $mline .= '<tr><td><a href="http://localhost/store-exact/addproducts.php?product_id='.$output.'" style="background: #09F; border-radius: 4px; box-shadow: none; color: #FFF; height: 45px; font-weight: 700; font-size: 16px; cursor: pointer; text-align: center; text-decoration: none;  border: 1px solid #166F8E; border-spacing: 0; line-height: 45px; width:300px; margin:0px auto 40px; display:block;">Checkout Now</a></td></tr>';
	   
	  
	  $cart_contents = $products_line . $mline;
	  
	  $footer = '<tr><td><hr style="margin-bottom:20px;"></td></tr>
	  <tr><td style="text-align:center;"><span style="font-size:20px;">Jupiter Kiteboarding</td></tr>
	  <tr><td style="text-align:center;"><a style="color:000;" href="mailto:customersupport@jupiterkiteboarding.com">customersupport@jupiterkiteboarding.com</a></td></tr>
	  <tr><td style="text-align:center;"><a style="color:000;" href="tel:5614270240">561-427-0240</a></td></tr>';

		// E-mail Processing - Requires EMAIL_* defines in the
		// includes/languages/english/recover_cart_sales.php file
		$cquery = tep_db_query("select * from customers where customers_id = '".$cid."'" );
		
	  $email2 = $email.$cart_contents . $footer;
	  
	  $ei_search = array("\n" ,"\r");

	  $ei_html_email = str_replace($ei_search, "", $email2);
	
		$email .= "\n\n". $_POST['message'];
		
		$inrec34 = tep_db_fetch_array($cquery);
		$custname = $inrec34['customers_firstname'].'&nbsp;'.$inrec34['customers_lastname'];

		$outEmailAddr = $inrec34['customers_email_address'];
		
	if ($action == 'email'){
		tep_mail($custname, $outEmailAddr, 'Complete your purchase at Jupiter Kiteboarding', $ei_html_email, '', STORE_OWNER . EMAIL_FROM);
...
?>
 ...
 <?php echo $action. $cid . $custid; ?>
             
                <form method=post action=<? echo $PHP_SELF;?> >
                  <table align="right" width="100%">
                    <tr class="dataTableContent" align="right">
                      <td><?php echo DAYS_FIELD_PREFIX; ?><input type=text size=4 width=4 value="0" name=sdate> - <input type=text size=4 width=4 value=<?php echo $tdate; ?> name=tdate><?php echo DAYS_FIELD_POSTFIX; ?><input type=submit value="<?php echo DAYS_FIELD_BUTTON; ?>"></td>
                    </tr>
                  </table>
                </form>
        
<form method=post id="recovered">
  <table border="0" width="100%" cellspacing="0" cellpadding="2">
            <tr class="dataTableHeadingRow">
              ...
              <td class="dataTableHeadingContent" align="left" colspan="2" width="15%" nowrap></td>
            </tr><tr>&nbsp;<br></tr>
            <tr class="dataTableHeadingRow">
...
$beg_date = date("Ymd", strtotime($bdate.'24hours'));
 //$bdate = seadate($sdate);
 $ndate = seadate($tdate);
 $query1 = tep_db_query("select cb.customers_id cid,
                         ...
                         from   " . TABLE_CUSTOMERS_BASKET . " cb,
                                " . TABLE_CUSTOMERS . " cus
                         where  cb.customers_basket_date_added < '" . $beg_date . "' and
...
--------
      ...
      if ($curcus != "")
		{
			...
			$cline = "
				<tr bgcolor=" . $fcolor . ">
				<td class='dataTableContent' align='center' width='1%'>" . tep_draw_checkbox_field('custid['.$curcus.']', $curcus, RCS_AUTO_CHECK == 'true' ? $checked : 0) . "</td>
				...
				<td class='dataTableContent' align='left' colspan='2' width='30%' nowrap>'". $inrec['email'] . "'</a></td>
				<td class='dataTableContent' align='left' colspan='2' width='15%' nowrap></td>
				</tr>";
		}
    }
  ...
  echo $cline;
  echo'</table>';
 echo '<hr size=1 color=000080><div class="col-xs-12 form-group" style="margin-top:20px;"><b>'. PSMSG .'</b><br>'. tep_draw_textarea_field('message', 'soft', '80', '5') .'<br>
 <span style="float: left;width: 100%; margin-top:15px;"><label role="checkbox" for="discount"><input type="checkbox" name="discount_code" value="1" id="discount">Include Discount Code in Emails</label></span>
 <a style="background:#ddd; display:none;" class="form-horizontal ui-widget btn" onclick="sendEmail();">Send Emails</a></div>';
...
?>
 ...
 <script>
 function sendEmail(){
var data = $('#recovered').serialize();
 $.ajax({
  type : 'POST',
  url  : 'recover_cart_sales.php?action=email',
  data : data,
