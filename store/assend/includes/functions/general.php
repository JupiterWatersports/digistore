...
function tep_admin_check_login() {
  ...
  if (!tep_session_is_registered('login_id')) {
    ...
    if ($filename != FILENAME_DEFAULT && $filename != FILENAME_FORBIDEN && $filename != FILENAME_LOGOFF && $filename != FILENAME_ADMIN_ACCOUNT && $filename != FILENAME_POPUP_IMAGE && $filename != 'packingslip.php' && $filename != 'invoice.php' && $filename != 'manager_override.php') {
...
function tep_get_category_link_name($category_id) {
    $category_query = tep_db_query("select categories_link_name from " . TABLE_CATEGORIES_DESCRIPTION . " where categories_id = '" . (int)$category_id . "'");
    $category = tep_db_fetch_array($category_query);
    return $category['categories_link_name'];
  } 
...
// Retreive server information
  ...
  function tep_remove_product($product_id) {
    ...
    tep_db_query("delete from " . TABLE_DISCOUNTPLUS . " where products_id = '" . (int)$product_id . "'");
...
while ($order = tep_db_fetch_array($order_query)) {
$options_query = tep_db_query("SELECT products_options_id, products_options_values_id FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " ON products_options_name = products_options LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON products_options_values_name = products_options_values WHERE orders_products_id = '" . $order['orders_products_id'] . "'");
...
// fixing update by syswatis
$get_quantity_query = tep_db_query("select sum(options_quantity) as total_quantity from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id='" . $order['products_id'] . "'");
$get_quantity = tep_db_fetch_array($get_quantity_query);
$total_quantity = $get_quantity['total_quantity'];
tep_db_query("update " . TABLE_PRODUCTS . " set products_quantity = '" . $total_quantity . "' where products_id = '" . $order['products_id'] . "'");
tep_db_query("update " . TABLE_PRODUCTS . " set products_ordered = products_ordered - " . $order['products_quantity'] . " where products_id = '" . $order['products_id'] . "'");
// end fixing update
}
}
...
//    fwrite($fp_err, date('Y-m-d H:i:s').$cache_block."\n\n"); //add timestamp to the verbose log
//    for ($i=0, $n=sizeof($cache_blocks); $i<$n; $i++) {
//    	fwrite($fp_err, 'in cache block '.$cache_blocks[$i]['code']."\n\n");
//      if ($cache_blocks[$i]['code'] == $cache_block) {
//        if ($cache_blocks[$i]['multiple']) {
//          if ($dir = @opendir(DIR_FS_CACHE)) {
//            while ($cache_file = readdir($dir)) {
//              $cached_file = $cache_blocks[$i]['file'];
//              fwrite($fp_err, 'cache file '.$cache_blocks[$i]['file']."\n\n");
//              $languages = tep_get_languages();
//              for ($j=0, $k=sizeof($languages); $j<$k; $j++) {
//                $cached_file_unlink = ereg_replace('-language', '-' . $languages[$j]['directory'], $cached_file);
//                fwrite($fp_err, 'cache file unlinked '.$cached_file_unlink."\n\n");
//                if (ereg('^' . $cached_file_unlink, $cache_file)) {
//                	fwrite($fp_err, 'unlinking '.$cache_file."\n\n");
...
function tep_get_years($default = '') {
    $years_array = array();
    if ($default) {
      $years_array[] = array('id' => '',
                                 'text' => $default);
    }
    $years_query = tep_db_query("select distinct YEAR(date_purchased) as y from " . TABLE_ORDERS . " order by year(date_purchased) desc");
    while ($years = tep_db_fetch_array($years_query)) {
      $years_array[] = array('id' => $years['y'],
                                 'text' => $years['y']);
    }
    return $years_array;
  }
...
function rdel($path, $deldir = true) { 
        ...
        // $deldir (param�tre optionel, par d�faut � vrai) permet de dire si vous souhaitez supprimer le r�pertoire (vrai) ou le vider uniquement (faux) 
  
        // on v�rifie d'abord que le nom du repertoire contient "/" � la fin, sinon on le lui rajoute 
        ...
        if (is_dir($path)) { 
                ...
                while ($f = readdir($d)) { 
                        if ($f != "." && $f != "..") { 
                                ...
                                if (is_dir($rf)) // si c'est un r�pertoire on appel r�cursivement la fonction 
                                        rdel($rf); 
                ...
                if ($deldir) // si $deldir est vrai on efface le r�pertoire 
                        rmdir($path); 
        } 
        else { 
                unlink($path); 
...
function epf_build_ptype_pulldown($parent_id = 0, $value_array = '', $indent = '') {
    ...
    $sql = tep_db_query("select d.ptype_id, ptype_description from " . TABLE_PTYPES . " t, " . TABLE_PTYPE_DESC . " d where t.ptype_id = d.ptype_id and languages_id = " . (int)$languages_id . " and parent_id = " . (int)$parent_id . " order by sort_order, ptype_description");
      ...
      while ($v = tep_db_fetch_array($sql)) {
          $check_what_category_query = tep_db_query("select master_ptype_id from product_types where ptype_id = '".$v['ptype_id']."'");
          $check_what_category = tep_db_fetch_array($check_what_category_query);
          
          $value_array[] = array('data-id'=> $check_what_category['master_ptype_id'], 'id' => $v['ptype_id'], 'text' => $indent . $v['ptype_description']);
...
// TABLES: countries;
  ...
  function tep_barcode_applet($url) {
    
    $query_config = tep_db_query("select active, port from " . TABLE_BARCODE_CONFIG);
	  $result_config = tep_db_fetch_array($query_config);
    
    if ($result_config['active'] == 'Y')
    {
      echo '<applet codebase="barcode/"';
      echo '        archive="SBarcode.jar"';
      echo '        code="RMIBarcode.BarcodeApplet"';
      echo '        width=100 height=100>';
      echo '  <param name=url VALUE="' . $url . '">';
      echo '  <param name=port VALUE="' . $result_config['port'] . '">';
      echo '</applet>';
    }
  }
...
// Function to add or remove quantities from product stock
  function tep_restock_order($order_id, $restock = false, $products_id=false, $qtydif = false) {
	
    if ($restock == 'add') {
	$order_query = tep_db_query("select products_id, products_quantity, orders_products_id from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'".($products_id?" and orders_products_id='".(int)$products_id."' ":""));
		while($order = tep_db_fetch_array($order_query)){
			if($qtydif) $order['products_quantity'] = $qtydif;
	$options_query = tep_db_query("SELECT products_options_id, serial_no, products_options_values_id FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " ON products_options_name = products_options LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON products_options_values_name = products_options_values WHERE orders_products_id = '" . $order['orders_products_id'] . "'");
			while($option = tep_db_fetch_array($options_query)){
	//$option = tep_db_fetch_array($options_query);
			tep_db_query("update " . TABLE_PRODUCTS_ATTRIBUTES . " set options_quantity = options_quantity + " . $order['products_quantity'] . " where products_id = '" . $order['products_id']. "' AND options_id = '" . (int)$option['products_options_id'] . "' AND options_values_id = '" . $option['products_options_values_id'] ."' AND options_serial_no = '".$option['serial_no']. "'");
			}
	// END FIXING BUG TAAJNY
	// fixing update by syswatis
	/*$get_quantity_query = tep_db_query("select sum(options_quantity) as total_quantity from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id='" . $order['products_id'] . "'");
	...
	$order_query = tep_db_query("select products_id, products_quantity, orders_products_id from " . TABLE_ORDERS_PRODUCTS . " where orders_id = '" . (int)$order_id . "'".($products_id?" and orders_products_id='".(int)$products_id."' ":""));
		...
		$options_query = tep_db_query("SELECT products_options_id, serial_no, products_options_values_id FROM " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " LEFT JOIN " . TABLE_PRODUCTS_OPTIONS . " ON products_options_name = products_options LEFT JOIN " . TABLE_PRODUCTS_OPTIONS_VALUES . " ON products_options_values_name = products_options_values WHERE orders_products_id = '" . $order['orders_products_id'] . "'");
			while($option2 = tep_db_fetch_array($options_query)){
	//$option = tep_db_fetch_array($options_query);
			tep_db_query("update " . TABLE_PRODUCTS_ATTRIBUTES . " set options_quantity = options_quantity - " . $order['products_quantity'] . " where products_id = '" . $order['products_id']. "' AND options_id = '" . (int)$option2['products_options_id'] . "' AND options_serial_no = '".$option2['serial_no']. "' AND options_values_id = '" . $option2['products_options_values_id'] ."'");
			}
...
function get_status_comments($order_id){
	$table = '<table class="table table-striped" id="commentsTable">
	<thead>
		<tr class="dataTableHeadingRow">
    		...
    		<th class="dataTableHeadingContent" align="center">Customer Notified</th>
	...
	<tbody>';
	$orders_history_query = tep_db_query("SELECT osh.orders_status_history_id, s.orders_status_name, osh.date_added, osh.customer_notified, osh.comments FROM orders_status_history osh, orders_status s WHERE osh.orders_id = '" . tep_db_prepare_input($order_id) . "' AND osh.orders_status_id = s.orders_status_id ORDER BY osh.date_added");
	
	if (tep_db_num_rows($orders_history_query)) {
		while ($orders_history = tep_db_fetch_array($orders_history_query)) {
$table .= '<tr id="commentRow' . $orders_history['orders_status_history_id'] . '">
            ...
            $table .= '<i class="fa fa-quote-right" style="margin-right:5px;"></i><i class="fa fa-envelope-o"  style="margin-right:5px;"></i></td>';
          } elseif ($orders_history['customer_notified'] == '3') {
            $table .='<i class="fa fa-envelope-o" style="margin-right:5px;"></i></td>';
          } elseif ($orders_history['customer_notified'] == '1') {
            $table .= tep_image(DIR_WS_ICONS . 'tick.gif', ICON_TICK) . '</td>';
          } else {
            $table .= tep_image(DIR_WS_ICONS . 'cross.gif', ICON_CROSS) . '</td>';
          }
          $table .='<td class="align-middle" align="left">'.$orders_history['orders_status_name'].'</td>';
          
			if ($orders_history['customer_notified'] == '4') {
              $table .='<td class="align-middle"><div style="display:table; padding:10px;">'.nl2br($orders_history['comments']).'</div>';
          } elseif ($orders_history['comments'] =='Tracking Sent' ){
             $table .='<td class="align-middle"><div style="display:table; padding:25px; width:100%;">Tracking Sent</div>';
          } else {
			$table .='<td class="align-middle" align="left">' . tep_draw_textarea_field("update_comments[" . $orders_history['orders_status_history_id'] . "][comments]", "soft", "40", "5", "".	tep_db_output($orders_history['comments']) . "", "onChange=\"updateCommentsField('update', '" . $orders_history['orders_status_history_id'] . "', 'false', encodeURIComponent(this.value))\"") . '';
          }
		}
	} else {
      $table .='  <tr>'.
       '    <td class="align-bottom" colspan="5">' . TEXT_NO_ORDER_HISTORY . '</td>'.
       '  </tr>';
	}
  $table .= '</table>';
	return $table;
