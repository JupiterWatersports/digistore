...
function linkcheck($path, $engine) {
  ...
  if(!file_exists($path)) {
		$data = strtolower(strip_tags(implode("", (array)GetFileArray($path))));
...
function get_link_popularity($link_url) {
 ...
 if($link_url) {	
	...
	if(!file_exists($path)) {
		$data = strtolower(implode("", (array)GetFileArray($path)));
		$data = substr($data, strpos($data, "of about")+9, strlen($data));
	...
	if(!file_exists($path)) {
		$data = strip_tags(implode("", (array)GetFileArray($path)));
	...
	if(!file_exists($path)) {
		$data = implode("", (array)GetFileArray($path)); //echo $data; // TEST
