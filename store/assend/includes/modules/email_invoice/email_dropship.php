...
define('EMAIL_SEPARATOR', '------------------------------------------------------');
	...
	if (EMAIL_USE_HTML == 'true') {
		...
		ob_start();
		include('templates/dropship.php');
	...
	// Build the standard email using OSC code
  	$email_order = 	STORE_NAME . "\n" . 
                 		...
                 		EMAIL_TEXT_INVOICE_URL . ' ' . tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$oID, 'SSL', false) . "\n" .
                 		EMAIL_TEXT_DATE_ORDERED . ' ' . strftime(DATE_FORMAT_LONG) . "\n\n";
  	
	 if (isset($_GET['comments'])) {
            
        $email_order .= tep_db_output($_GET['comments']) . "\n\n";
      }
	...
	$email_order .= 	EMAIL_TEXT_PRODUCTS . "\n" . 
  	...
  	$email_order .= "\n" . 	EMAIL_TEXT_BILLING_ADDRESS . "\n" .
                 			 EMAIL_SEPARATOR . "\n" .
                  		tep_address_format($order->customer['format_id'], $order->customer, 1, '', '<br>') . "\n\n";
	...
	define('EMAIL_TEXT_SUBJECT','Drop Ship Request');
	$ei_message->build_message();
	$ei_message->send('', $_GET['email-to'], STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, EMAIL_TEXT_SUBJECT);
	$ei_message->send('', 'customersupport@jupiterkiteboarding.com', STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, 'CONFIRMATION');
