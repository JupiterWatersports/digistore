...
// -Country-State Selector
      ...
      case 'deleteconfirm':
        ...
        break;
      default:
        $customers_query = tep_db_query("select c.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
...
<script type="text/javascript" src="ext/jquery/ui/search_cust_controller.js"></script>
<link rel="stylesheet" type="text/css" href="search_live.css" />
</head>
...
?>
		  ...
		  <h1 class="pageHeading"><?php echo HEADING_TITLE; ?></h1>
            <div style="margin:10px 0px 10px 0px;"><?php echo HEADING_TITLE_SEARCH . ' '; ?><div style="position:relative; width:160px;"><input type="text" name="search" class="form-control" id="search" value="" autocomplete="off"><div id="HeadresultsContainer"></div></div></div>
          ...
          <tr>
            ...
            <thead>
              <tr class="dataTableHeadingRow">
                <th align="left" class="dataTableHeadingContent"><?php echo TABLE_HEADING_LASTNAME; ?></th>
                <th align="left" class="dataTableHeadingContent"><?php echo TABLE_HEADING_FIRSTNAME; ?></th>
                <th align="left" class="dataTableHeadingContent">Phone Number</th>
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search_id']) && tep_not_null($HTTP_GET_VARS['search_id'])) {
      $keywords = tep_db_input(tep_db_prepare_input($HTTP_GET_VARS['search_id']));
      $search = "where c.customers_id = '" . $keywords . "'";
    }
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_telephone, c.customers_email_address, a.entry_country_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id " . $search . " order by c.customers_lastname, c.customers_firstname";
...
?>
                ...
                <td class="dataTableContent"><?php echo '<a href="tel:'.$customers['customers_telephone'].'">'.$customers['customers_telephone']. '</a>'; ?></td>
  ...
  switch ($action) {
    ...
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $heading[] = array('text' => '<div style="text-align:center; line-height:0px; padding-bottom:10px;">'. tep_draw_separator('pixel_trans.gif', '11', '12') .'&nbsp;<br><b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</div></b>');
        ...
        $contents[] = array('align' => 'center', 'text' => '<div style="width:200px;">
		<div class="fourbuttons"><a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a></div>
		<div class="fourbuttons"><a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a></div> 
		<div class="fourbuttons"><a href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_orders.gif', IMAGE_ORDERS) . '</a></div>
		<div class="fourbuttons"><a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', IMAGE_EMAIL) . '</a></div>
		<div class="fourbuttons"><a href="' . tep_href_link('password_forgotten.php', 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_send_new_password.gif', 'Send New Password') . '</a></div>
		<div class="fourbuttons"><a href="' . tep_href_link(FILENAME_CREATE_ORDER) . '?Customer='. $cInfo->customers_id .'">' . tep_image_button('button_create_order.gif', IMAGE_CREATE_ORDER) . '</a></div></div>');
		if (MAILCHIMP_ENABLE == true) {
		$contents[] = array('align' => 'center', 'text' => ' <a style="display:none;" href="' . tep_href_link(FILENAME_CUSTOMERS, 'action=mailchimp') . '">' . tep_image_button('button_mc_sync.gif', 'Mailchimp Sync') . '</a> ');
		}
