...
/*
  ...
  */
  ...
  if(isset($_POST['action']) && $_POST['action']=='ajaxupdate'){
	...
	if($product &&  $pid){
		$current_stock = $product['newstock'];
		$sql = "UPDATE products SET products_quantity = '".$current_stock."', products_model = '".$product['model']."', products_price = '".$product['price']."', products_upc = '".$product['upc']."', manufacturers_id = '".$product['manufacturer']."', products_status = '".$product['active']."' WHERE products_id = ".$pid;
		tep_db_query($sql );
	}
	if($options &&  $pid){
		foreach($options as $opt){
			if($opt['value_quantity'] != ''){
				$sql = "UPDATE products_attributes SET options_quantity = '".$opt['value_quantity']."'  WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']." and products_attributes_id='".(int)$opt['attributes_id']."' ";
				tep_db_query($sql );
			}
/* extra fields added begin */
			if($opt['value_upc'] != ''){
				$sql = "UPDATE products_attributes SET options_upc = '".$opt['value_upc']."' WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']." and products_attributes_id='".(int)$opt['attributes_id']."' ";
				tep_db_query($sql );
			}
			if($opt['value_optmodel'] != ''){
				$sql = "UPDATE products_attributes SET options_model_no = '".$opt['value_optmodel']."' WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']." and products_attributes_id='".(int)$opt['attributes_id']."' ";
				tep_db_query($sql );
			}
			if($opt['value_serialno'] != ''){
				$sql = "UPDATE products_attributes SET options_serial_no = '".$opt['value_serialno']."' WHERE options_id='".(int)$opt['option_id']."' and options_values_id = ".(int)$opt['value_id']." and products_attributes_id='".(int)$opt['attributes_id']."' ";
				tep_db_query($sql );
			}
/* extra fields added end */
  ...
  //Credit to surfalot (Run SQL Script)
	...
	function qs_update_db($qs_file,$isfile=true){
		if(!$isfile) 
			$restore_query = $qs_file;
		else{
...
// join the query before the comment appeared, with the rest of the dump
				...
				if ( (preg_match('/alter /i', $next)) || (preg_match('/update/i', $next)) || (preg_match('/create/i', $next)) || (preg_match('/insert/i', $next)) || (preg_match('/drop t/i', $next)) ) {
 ...
 // Function to create drop down list for category selection - Added 2008/12/30 Azrin Aris
  ...
  function tep_quickstock_product_listing($cat_id){
  	if (tep_not_null($cat_id)) {
			$sql2 = tep_db_query("SELECT p.products_model, p.products_id, p. products_quantity, p.products_status, p.products_upc, p.products_price, p.manufacturers_id, pd.products_name from products p, products_to_categories ptc, products_description pd where p.products_id = ptc.products_id and p.products_id = pd.products_id  and pd.language_id = '1'and ptc.categories_id = '" . $cat_id . "'  order by p.products_status DESC, pd.products_name");
...
?>
         ...
         <td class="dataTableContent" align="center"><input style="width:170px;" type="text" size="3" name="stock_update[<?php echo $results['products_id'] ?>][upc]" value="<?php echo $results['products_upc'] ?>" onChange="<?php echo $doChange?>" class="prodfield_<?php echo  $results['products_id'] ?>" ></td>
...
<td align="left" class="smallText">&nbsp;Serial No:&nbsp;<input  style="width:170px;" name="stock_update[opt][<?php echo $products_options['products_options_values_id'];?>][<?php echo $products_options['options_id'].'-'.$results['products_id'];?>][value_serialno]" type="text" class="prodfield_<?php echo  $results['products_id'] ?>"   value="<?php echo $products_options['options_serial_no']; ?>" size="6"><input  name="stock_update[opt][<?php echo $products_options['products_options_values_id'];?>][<?php echo $products_options['options_id'].'-'.$results['products_id'];?>][attributes_id]" type="hidden" class="prodfield_<?php echo  $results['products_id'] ?>"   value="<?php echo $products_options['products_attributes_id']; ?>" size="6"></td>
<!-- extra fields added end-->
</tr>
<?php
...
?>
  ...
  if(tep_not_null($stock_update)){
		...
		foreach($stock_update['opt'] as $value_id => $inf){
			
			foreach($inf as $k=>$v){
				$vk = $value_id.'-'.$v['attributes_id'];
				if(!isset($options[$vk])) $options[$vk]=array();
				$options[$vk]['value_id']=$value_id;
				$k=explode('-',$k);
				$options[$vk]['option_id']=$k[0];
				$options[$vk]['pid']=$k[1];
				foreach($v as $v1=>$v2) $options[$vk][$v1]=$v2;
			}
		} 
		   
		if($options){
			foreach($options as $opt){
				//if($stock_update[$opt['pid']]['changed']==0) continue;
	...
	/* extra fields added end */
		 ...
		 while (list($key, $items) = each($stock_update)){
		   ...
		   if($action_type==0)
		   {
			  ...
			  if($changed){
				  ...
				  if($filter_type==1){
					  $sql = "UPDATE products SET products_quantity = '".$current_stock."', products_model = '".$items['model']."', products_price = '".$items_price."', products_upc = '".$items['upc']."', manufacturers_id = '".$items['manufacturer']."', products_status = '".$items['active']."' WHERE products_id = $key";
					  
					  $qs_sql .= "$sql;\n";
				  }
				  else {
					  $sql = "UPDATE products SET products_quantity = '".$current_stock."', products_model = '".$items['model']."', products_price = '".$items_price."', products_upc = '".$items['upc']."', products_status = '".$items['active']."' WHERE products_id = $key";
		...
		if($update_count){
			...
			$update_status = qs_update_db($qs_sql,false);
...
<!-- header_eof //-->
		...
		<h1 class="pageHeading"><?php echo QUICK_HEAD1; ?></h1>
    
      <form action="quick_stockupdate.php" method="post" name="CategorySelect_Form">

          <div style="height:20px;"></div><?php echo QUICK_HEAD2; ?>

 
        <p><?php echo tep_quickstock_category_selector(); ?></p>
          ...
          <td valign="top">
           ...
           <table border="0" width="100%" cellspacing="2" cellpadding="2">
            ...
            <tr class="dataTableHeadingRow" style="background-image:none;">
             ...
             <td class="dataTableHeadingContent" width="10%" align="center"> <?php echo QUICK_MANUFACTURER; ?></td>

             <td class="dataTableHeadingContent" width="40%" align="left"><?php echo QUICK_NAME; ?></td>

             <td class="dataTableHeadingContent" width="3%" align="center">UPC</td>

             <td class="dataTableHeadingContent" width="8%" align="center">Price</td>
