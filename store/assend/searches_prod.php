...
// Output a form checkbox menu for product info page
  function tep_draw_checkbox_menu($name, $values, $default = '', $parameters = '', $required = false, $oID='', $url_id='',$url_title='',$products_options_name='',$options_id='',$attributeserial='' ) {
	$field ='<div class="url-holder">
<div style="display:inline;float:left"><form action="'. tep_href_link(FILENAME_ORDERS_EDIT_ADD_PRODUCT, 'oID=' . $oID).'" method="POST"><input type="hidden" name="step" value="3">';
	$field .= '<input type="hidden" name="add_product_products_id" id="add_product_products_id" value="'. $url_id.'"><input type="hidden" name="product_search" id="product_search" value="'.$url_title.'">';
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
	  ...
	  $field .='<div class="url-holder">
<div style="display:inline;float:left">'.tep_draw_form('search', 'client_search.php', '', 'post');
$field .= '<input type="hidden" name="product_id" id="add_product_products_id" value="'. $url_id.'">';
$field .= '<input type="hidden" name="add_product_options_value" id="products_options_id" value="' . $value . '">';
$field .= '<input type="hidden" name="products_serial" value="'.$attributeserial . '">';

$field .= '<input type="text" name="product_search" id="product_search" value="' .$products_options_name .' '. tep_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '"';
        $field .= 'readonly style="width:400px; padding-left:20px; background:#fff; border:none; align:left;" onClick="this.form.submit();"><input type="submit" value="add" style="display:none;"></form></div></div>';
    }
    $field .= '';
...
<a style="width:100%;align:left;">'.tep_draw_form('search', 'client_search.php', '', 'post').'<input style="width:100%;" type="text" name="product_name" id="product_name" value="'. $url_title .'" readonly style="" onclick="this.form.submit();">';
echo '<input type="hidden" name="product_id" value="'.$url_url.'"></div>';
echo'
...
while ($row = mysqli_fetch_assoc($sqlresult))
{
  ...
  $products_attributes_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_ATTRIBUTES . " patrib where patrib.products_id='" . (int)$url_id . "' and patrib.options_id = popt.products_options_id and popt.language_id = '" . (int)$languages_id . "'");
    ...
    $attrib = tep_draw_checkbox_menu('id[' . $products_options_name['products_options_id'] . ']', $products_options_array, $selected_attribute,'','',$oID,$url_id,$url_title,$products_options_name['products_options_name'],$options_id, $attributeserial); 
