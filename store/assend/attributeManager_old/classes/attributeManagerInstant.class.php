...
//-----------------------------
	...
	function addAttributeToProduct($get) {
		...
		$data = array(
			...
			'options_quantity_id' => $quantity_id,
			'price_prefix' => $prefix
		);
...
// End QT Pro Plugin
	...
	function update($get) {
        ...
        if (AM_USE_MPW) {
          ...
          $data['options_quantity_id'] = $quantity_id;
...
//-----------------------------
			...
			//print_R($res);
				...
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['options_values_id']]['quantity_id'] = $res['options_quantity_id'];
