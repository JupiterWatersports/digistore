...
$sqlquery = "SELECT distinct(p.products_id),
                    ...
                    p.products_price,
		    p.products_upc,
                    p.products_tax_class_id, p.products_quantity
...
while ($row = mysqli_fetch_assoc($sqlresult))
{
 ...
 echo '
<div class="url-holder2" style="border-bottom:1px solid #ccc;">
<div><a style="font-size:1rem;" href="' . $url_url . '" class="url-title">' . $url_title .'</a></div>
<div style="margin-left:20px; font-size:0.8rem; margin-bottom:10px;"><b>Current Stock:</b> '.$row['products_quantity'].'</div>
</div>
...
while ($row = mysqli_fetch_assoc($sqlresult))
{
  $url_title = htmlspecialchars($row['products_name']);
  $url_id = $row['products_id'];
   $url_url = "categories.php?pID=" . $row['products_id']."&action=new_product";
