...
<script type="text/javascript" src="ext/jquery/ui/controller_order.js"></script>
        ...
        <td><table border="0" cellspacing="0" cellpadding="2" style="border: 1px solid #C9C9C9;" align="center">
          ...
          <tr class="dataTableRow">
           <form action="<?php echo tep_href_link(FILENAME_ORDERS_EDIT_ADD_PRODUCT, 'oID=' . $_GET['oID']); ?>" method="POST">
            <td class="dataTableContent" align="right" width="50"><?php echo TEXT_STEP_1; ?></td>
            <td class="dataTableContent" valign="top"><?php echo tep_draw_pull_down_menu('add_product_categories_id', tep_get_category_tree('0', '', '0', $category_array), $add_product_categories_id,'style="width:270px;" onchange="this.form.submit();"'); ?></td>
...
tep_session_register(oID);
 $oID =$_GET['oID']; ?>
            <td>&nbsp;</td>
            <td class="dataTableContent" valign="top"><div id="form" >&nbsp;<input type="text" id="searchbox" name="product_search" value="<?php if(isset($_POST['product_search'])) echo $_POST['product_search']; ?>" autocomplete="off" onchange="this.form.submit();"></div><div id="resultsContainer"></div>
			</td>
...
?>
          ...
          <form action="<?php echo tep_href_link(FILENAME_ORDERS_EDIT_ADD_PRODUCT, 'oID=' . $_GET['oID']); ?>" method="POST">
            ...
            <td class="dataTableContent" valign="top"><?php echo tep_draw_pull_down_menu('add_product_products_id', $product_array, $add_product_products_id, 'style="width:270px;" onchange="this.form.submit();"'); ?></td>
  ...
  if (($step > 2) && ($add_product_products_id > 0)) {
    ...
    if ($has_attributes) {
      ...
      while ($products_options_name = tep_db_fetch_array($products_options_name_query)) {
        ...
        $products_options_query = tep_db_query("select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix, pa.options_quantity from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_OPTIONS_VALUES . " pov where pa.products_id = '" . (int)$add_product_products_id . "' and pa.options_id = '" . $products_options_name['products_options_id'] . "' and pa.options_values_id = pov.products_options_values_id and pov.language_id = '" . $languages_id . "'");
        while ($products_options = tep_db_fetch_array($products_options_query)) {
		$opt_qty='';
		if ($products_options['options_quantity'] <=0) $opt_qty='***';
          $products_options_array[] = array('id' => $products_options['products_options_values_id'], 'text' => $products_options_name['products_options_name'] .' - ' . $products_options['products_options_values_name']. ' '. $opt_qty);
        ...
        echo   '            <td class="dataTableContent" valign="top">' . tep_draw_pull_down_menu('add_product_options[' . $products_options_name['products_options_id'] . ']', $products_options_array, $selected_attribute, 'style="width:270px;"') . '</td>' . "\n" .
      ...
      echo '            <td class="dataTableContent">&nbsp;</td>' . "\n" .
           ...
           '          </tr>' . "\n" .
                          '<tr class="dataTableRow">' . "\n". 
			'<td class="dataTableContent">&nbsp;</td>' . "\n" .
			'<td class="dataTableContent" colspan="2" align="left">*** denotes out of stock</td>'.
           '          </tr>' . "\n" .
           '          </form>' . "\n";
    } else {
      $step = 4;
