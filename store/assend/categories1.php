              ...
              if (($e['language'] == $language_id))  {
                if ($e['language_active']) {
                  if ($e['multi_select']) {
                    if (empty($_POST[$e['field'] . '_' . $language_id])) {
                      ...
                      if ($e['linked']) { // validate linked values if field is linked
                        ...
                        if ($e['linked'] == 1) { // linked to a list field
                          foreach ($epf as $lf) {
                            if ($lf['id'] == $e['links_to']) {
                              $lv = (int)$_POST[$lf['field'] . '_' . $language_id];
                            }
                          }
                          if ($lv == 0) {
                    ...
                    $value = tep_db_prepare_input($_POST[$e['field'] . '_' . $language_id]);
                    if ($value == '')
                      $value = (($e['uses_list'] && !$e['multi_select']) ? 0 : 'null');
                  }
                  // if field is valid for the current product type store the value
                  if (empty($e['ptypes']) || in_array($_POST['products_type'], $e['ptypes'])) {
...
//                                  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$_GET['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
// original Extra Product Fields query
/*
      // begin Extra Product Fields
...
/*
      ...
      $product_query = tep_db_query("select pd.products_name, pd.products_description,
                                     ...
                                     p.products_item_weight, p.products_item_length, p.products_item_width, p.products_item_height,
      ...
      //TotalB2B end original
*/
      //TotalB2B start + Extra Product Fields


      $query = "select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_pdfupload, p.products_image,  p.products_image_xl_1, p.products_image_xl_2, p.products_image_xl_3,  p.products_image_xl_4, p.products_image_xl_5,  p.products_image_xl_6, p.products_price, p.products_cost, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_type";
      foreach ($xfields as $f) {
        $query .= ', pd.' . $f;
      }
      $query .= " from (".TABLE_PRODUCTS." p LEFT JOIN ".TABLE_PRODUCTS_IMAGES." pi ON p.products_id = pi.products_id)
                INNER JOIN ".TABLE_PRODUCTS_DESCRIPTION." pd ON p.products_id = pd.products_id
                where p.products_id = '" . (int)$_GET['pID'] . "'
                and pd.language_id = '".(int)$languages_id."'";
      $product_query = tep_db_query($query);
      //TotalB2B end + Extra Product Fields
    ...
    <div class="pages">
        ...
        <div class="pad">
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_separator('pixel_trans.png', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('products_type', epf_build_ptype_pulldown(0, array(array('id' => 0, 'text' => TEXT_NONE))), $pInfo->products_type, 'id="ptype" onChange="process_ptype_change()"'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.png', '1', '10'); // end Extra Product Fields?></td>
          </tr>
          <tr>
          ...
          foreach ($epf as $e) {
            for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
              if (($e['language'] == $languages[$i]['id']) && $e['language_active']) {
                ...
                if (tep_not_null($_POST)) { // load reposted value, back button was clicked in product preview
                  if (is_array($_POST[$input_field])) {
                    $currentval = $_POST[$input_field];
                  } else {
                    $currentval = stripslashes($_POST[$input_field]);
                  }
                ...
                if ($e['uses_list']) {
                   if ($e['multi_select']) {
                     ...
                     $inp = '';
                     if ($e['linked'] == 1) {
                       $tmp = (tep_not_null($_POST) ? stripslashes($_POST['extra_value_id' . $e['links_to'] . '_' . $languages[$i]['id']]) : tep_get_product_extra_value($e['links_to'], $pInfo->products_id, $languages[$i]['id']));
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
      $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_image_med, p.products_image_lrg, p.products_image_sm_1, p.products_image_xl_1, p.products_image_sm_2, p.products_image_xl_2, p.products_image_sm_3, p.products_image_xl_3, p.products_image_sm_4, p.products_image_xl_4, p.products_image_sm_5, p.products_image_xl_5, p.products_image_sm_6, p.products_image_xl_6, p.products_msrp, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_sort_order  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
// EOF MaxiDVD: Modified For Ultimate Images Pack!
      // begin Extra Product Fields
      $query = "select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_image_med, p.products_image_lrg, p.products_image_sm_1, p.products_image_xl_1, p.products_image_sm_2, p.products_image_xl_2, p.products_image_sm_3, p.products_image_xl_3, p.products_image_sm_4, p.products_image_xl_4, p.products_image_sm_5, p.products_image_xl_5, p.products_image_sm_6, p.products_image_xl_6, p.products_msrp, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_sort_order ";
      foreach ($xfields as $f) {
        $query .= ', pd.' . $f;
      }
         ...
         foreach ($epf as $e) {
           if (($e['language'] == $languages[$i]['id']) && $e['language_active']) {
             if (isset($_GET['read']) && ($_GET['read'] == 'only')) {
               ...
               if ($e['multi_select']) {
                 $value = $_POST[$e['field'] . '_' . $languages[$i]['id']];
               } else {
                 $value = tep_db_prepare_input($_POST[$e['field'] . '_' . $languages[$i]['id']]);
...
/* Re-Post all POST'ed variables */
      ...
      while (list($key, $value) = each($HTTP_POST_VARS)) {
        ...
        //echo tep_draw_hidden_field('products_head_keywords_tag[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($products_head_keywords_tag[$languages[$i
        } // end extra product fields
      }
