...
{	
	...
	function db_functions($server,$username,$password,$database,$tableprefix,$theme,$language)
	{
		...
		$this->lang=$language;
		$this->conn = mysql_connect("$server", "$username", "$password",true) or die("Could not connect : " . mysql_error());
	...
	function updateItemQuantity($item_id,$newQuantity)
	{
		...
		$query="UPDATE $items_table SET products_quantity=\"$newQuantity\" WHERE products_id=\"$item_id\"";
		mysql_query($query,$this->conn);
