...
class display
{
	...
	function display($connection,$dbf_oscommerce,$theme,$currency_symbol,$language)
	{
		$this->conn=$connection;
		$this->dbf_osc=$dbf_oscommerce;
		$this->lang=$language;
	...
	function displaySaleManagerTable($tableprefix,$where1,$where2)
	{
		...
		else
		{
			...
			$sales_items_tableheaders=array("{$this->lang->itemName}","{$this->lang->quantityPurchased}","{$this->lang->unitPrice}","{$this->lang->tax}","{$this->lang->itemTotalCost}","{$this->lang->updateItem}","{$this->lang->deleteItem}");
			$sales_items_tablefields=array('item_id','quantity_purchased','item_unit_price','item_total_tax','item_total_cost');
	...
	function displayTotalsReport($tableprefix,$total_type,$tableheaders,$date1,$date2,$where1,$where2)
	{
		...
		elseif($total_type=='items')
		{
			...
			$query="SELECT * FROM products as p,products_description as pd WHERE
			p.products_id=pd.products_id and pd.language_id=1 ORDER BY pd.products_name";
			$item_result=mysql_query($query,$this->dbf_osc);
			$temp_item_id=0;
		...
		elseif($total_type=='item')
		{
			...
			echo '</tr>'."\n\n";
				...
				$query="SELECT * FROM products as p,products_description as pd WHERE
				p.products_id=pd.products_id ORDER BY pd.products_name";
				$item_result=mysql_query($query,$this->dbf_osc);
	...
	{
		if($field=='unit_price' or $field=='total_cost' or $field=='buy_price' or $field=='sale_sub_total' or $field=='sale_total_cost' or $field=='item_unit_price' or $field=='item_total_cost' or $field=='item_total_tax' or $field=='products_price' )
		{
			return "$this->currency_symbol"."$data";
		}	
		elseif($field=='tax_percent' or $field=='tax_rate')
		{
			return "$data".'%';
		}
		...
		elseif($field=='item_id')
		{
			...
			p.products_id=\"$data\"";
			$result=mysql_query($query,$this->dbf_osc);
			$row=mysql_fetch_assoc($result);
