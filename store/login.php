...
// using guest_account with customers_email_address
   ...
   $check_customer_query = tep_db_query(  "select customers_id, customers_firstname, customers_password, customers_email_address, customers_default_address_id, guest_account from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($email_address). "' and customers_password IS NOT NULL");
...
<h3>Returning Customers</h3>
        ...
        <p class="p-1">Email Address</p>
      	<?php echo tep_draw_input_field('email_address','', 'class="form-control"'); ?></div>
        <div class="password col-xs-12">
        <p class="p-1">Password</p>
      	<?php echo tep_draw_password_field('password','', 'class="form-control"'); ?><br /><br />
<?php echo '<button id="login-btn" class="button-blue" style="display:inline-block">Login </button>'; ?> 
      	<?php echo '<div style="position:absolute; right:30px; bottom:8px;"><a href="' . tep_href_link(FILENAME_PASSWORD_FORGOTTEN, '', 'SSL') . '">Forgot your Password?</a></div>'; ?>
...
<div id="newcustomer">
 <div class="grid_4 omega" style="width:100%;"> 
      <div class="account-heading">
<h3>New Customers</h3>
      </div>
...
<div class="clear"></div>
 ...
 <div class="new-customers-buttons"> 
<?php echo '<a  href="' . tep_href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL') . '">'.'<button id="createaccount-btn"class="button-blue">Create an Account </button>' . '</a>'; ?> 
<?php echo '<a  href="' . tep_href_link(FILENAME_CREATE_ACCOUNT, 'guest=guest', 'SSL') . '">'.'<button id="guest-btn"class="button-blue">Checkout as Guest </button>' . '</a>'; ?> 
 </div>
   <?php
  // PWA BOF  
