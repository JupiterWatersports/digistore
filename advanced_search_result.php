...
// begin Extra Product Fields
       ...
       (isset($HTTP_GET_VARS['ptype']) && empty($HTTP_GET_VARS['ptype'])) &&
// end Extra Product Fields
...
// create column list
  ...
  $select_str = "select distinct " . $select_column_list . " m.manufacturers_id, p.products_id, pd.products_name, p.products_msrp, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price ";

  // begin Extra Product Fields
  $select_str .= ", p.products_type ";
  foreach ($epf as $e) {
...
// begin Extra Product Fields
  if (isset($HTTP_GET_VARS['ptype']) && tep_not_null($HTTP_GET_VARS['ptype'])) {
    $where_str .= " and (p.products_type in (" . implode(',', epf_get_ptype_children((int)$HTTP_GET_VARS['ptype'])) . "))";
  }
// end Extra Product Fields 
...
// begin Extra Product Fields
          if (isset($HTTP_GET_VARS['search_in_description']) && ($HTTP_GET_VARS['search_in_description'] == '1')) { // extra fields are part of product description and so should be searched only if searching in descriptions
            if ((PRODUCT_LIST_TYPE > 0) || (PTYPE_ON_INFO_PAGE != 'off')) { // search product type only if it is being displayed somewhere
              $type_query = tep_db_query("select ptype_id from " . TABLE_PTYPE_DESC . " where languages_id = " . (int)$languages_id . " and ptype_description like '%" . tep_db_input($keyword) . "%'");
              if (tep_db_num_rows($type_query) > 0) { // keyword is found in at least one type description
                $id_list = array();
                while ($tid = tep_db_fetch_array($type_query)) {
                  if ((PTYPE_ON_INFO_PAGE == 'basic') || ((PRODUCT_LIST_TYPE > 0) && (PTYPE_ON_INFO_PAGE == 'off'))) { // only basic type is ever seen
                    $id_list[] = $tid['ptype_id'];
                  } else { // extended type is shown on product information page, value would also be seen on any product that is a subtype of the found type
                    $id_list = array_merge($id_list, epf_get_ptype_children($tid['ptype_id']));
                  }
                }
                $id_list = array_unique($id_list);
                $where_str .= " or (p.products_type in (" . implode(',', $id_list) . "))";
              }
            }
            foreach ($epf as $e) {
              if ($e['uses_list']) {
  ...
  if ( (!isset($HTTP_GET_VARS['sort'])) || (!ereg('[1-8][ad]', $HTTP_GET_VARS['sort'])) || (substr($HTTP_GET_VARS['sort'], 0, 1) > sizeof($column_list)) ) {
    ...
    switch ($column_list[$sort_col-1]) {
      ...
      case 'PRODUCT_LIST_MODEL':
        ...
        break;
      case 'PRODUCT_LIST_TYPE': // begin Extra Product Fields
        $order_str .= "p.products_type " . ($sort_order == 'd' ? "desc" : "") . ", pd.products_name";
        break; // end Extra Product Fields
...
// stats_keywords.php_bof
  ...
  tep_db_query("update " . TABLE_SEARCH_QUERIES . " set search_count = search_count+1 where search_text = '" . $HTTP_GET_VARS['keywords'] . "'");
